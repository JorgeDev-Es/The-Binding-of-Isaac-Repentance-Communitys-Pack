...
declare interface EIDInterface {
	 ...
	 * @param ID ID and ownedID can be a collectible ID or a full item string.
	 * @param ownedID ownerID can also be a function rather than just an ID; if it returns true, the text will be displayed.
	 * @param text The text will be added as a new line, with the owned item's icon at the start.
	 * @param replaceText If you pass in replaceText, instead the text is found in the description and replaced with replaceText.
	 * @param language
	 * @param extraTable
	 */
	addCondition: (
		IDs: int | string | Array<string | int>,
		ownedID: int | string | (() => boolean),
		text,
		replaceText?,
		language?: languages,
	 ...
	 * @param modText
	 * @param extraTable
	 * @param checkInReminder default true
	 */
	addItemConditional: (
		IDs: int | string | Array<string | int>,
		ownedIDs: int | string | Array<string, int>,
		modText: string,
		extraTable?: extraTable,
	 ...
	 * @param language
	 */
	newModdedCondition: (text: string, language) => string
}
