-- Originally created by Sanio (Sanio#5230)
-- This version modified quite a lot by Connor
...
local function GetScreenTopLeft()
	...
	return Vector(hudOffset * 20, hudOffset * 12)
end
...
-- Returns a sprite offset for whenever Book of Virtues or Judas' Birthright is active.
-- The charge bar has a slightly different offset.
local function GetBookOffset(player, activeSlot, isChargeBar)
	if activeSlot == ActiveSlot.SLOT_PRIMARY then
		local playerType = player:GetPlayerType()

		local judasBirthrightActive = (playerType == PlayerType.PLAYER_JUDAS or playerType == PlayerType.PLAYER_BLACKJUDAS) and
			player:HasCollectible(CollectibleType.COLLECTIBLE_BIRTHRIGHT)

		local bookOfVirtuesActive = player:HasCollectible(CollectibleType.COLLECTIBLE_BOOK_OF_VIRTUES)
			and player:GetActiveItem(ActiveSlot.SLOT_PRIMARY) ~= CollectibleType.COLLECTIBLE_BOOK_OF_VIRTUES

		if judasBirthrightActive or bookOfVirtuesActive then
			return isChargeBar and Vector(0, 3) or Vector(0, -4)
		end
	end
...
function renderActive:Add(tab)
	if not tab.Sprite and tab.Anm2Filename then
		tab.Sprite = Sprite()
	...
	if not tab.GetSprite then
		tab.GetSprite = function(this)
			return this.Sprite
		end
	end
	tab.Offset = tab.Offset or Vector(0,0)
	tab.Scale = tab.Scale or Vector(1,1)
...
local function UpdateSpriteColor(playerIndex, activeSlot, params)
	...
	local sprite = params:GetSprite(player, activeSlot)
				...
				if params.RenderAbove == renderAbove and (not params.ItemID or player:HasCollectible(params.ItemID)) then
					...
					for k = 1, #slots do
						...
						if ((params.Condition == nil) or (params.Condition ~= nil and params.Condition(player, activeSlot) == true)) then
							...
							local sprite = params:GetSprite(player, activeSlot)
							sprite.Scale = params.Scale * size
							local offset = Vector(params.Offset.X, params.Offset.Y) + GetBookOffset(player, activeSlot, params.IsChargeBar)
							if params.IsChargeBar then
								offset = offset + GetChargebarOffset(player, activeSlot)
							end
							sprite.Offset = offset * size
