...
local function getStackedCrucifixDuration(player, secondHandMultiplier)
	local base = 150 
	local result = math.ceil(base * (math.log(math.max(1, player:GetCollectibleNum(mod.ITEM.COLLECTIBLE.CRUCIFIX)), 5) + 1))
...
function mod:crucifixOnFireTear(player, tear, secondHandMultiplier)
	if player:HasCollectible(FiendFolio.ITEM.COLLECTIBLE.CRUCIFIX) then
		...
		data.ApplyMartyrDuration = getStackedCrucifixDuration(player, secondHandMultiplier)
...
function mod:crucifixOnKnifeDamage(player, entity, secondHandMultiplier)
	if player:HasCollectible(FiendFolio.ITEM.COLLECTIBLE.CRUCIFIX) then
		FiendFolio.MarkForMartyrDeath(entity, player, getStackedCrucifixDuration(player, secondHandMultiplier), false)
	end
end
