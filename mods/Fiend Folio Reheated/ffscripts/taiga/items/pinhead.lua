...
local function isPinheadUseSuccessful(player)
	local chance = math.max(5, 10 + (player:GetCollectibleNum(mod.ITEM.COLLECTIBLE.PINHEAD) * 10) + (player.Luck * 5))
...
local function getStackedPinheadDuration(player, secondHandMultiplier)
	local base = 210 
	local result = math.ceil(base * (math.log(math.max(1, player:GetCollectibleNum(mod.ITEM.COLLECTIBLE.PINHEAD)), 5) + 1))
...
function mod:pinheadOnFireTear(player, tear, secondHandMultiplier)
	if player:HasCollectible(FiendFolio.ITEM.COLLECTIBLE.PINHEAD) then
		if isPinheadUseSuccessful(player) then
			...
			tear:GetData().ApplySewnDuration = getStackedPinheadDuration(player, secondHandMultiplier)
		end
	end
end
...
function mod:pinheadOnFireBomb(player, bomb, secondHandMultiplier)
	if player:HasCollectible(FiendFolio.ITEM.COLLECTIBLE.PINHEAD) then
		if isPinheadUseSuccessful(player) then
			...
			data.ApplySewn = true
			data.ApplySewnDuration = getStackedPinheadDuration(player, secondHandMultiplier)
		end
	end
end
...
function mod:pinheadOnKnifeDamage(player, entity, secondHandMultiplier)
	if player:HasCollectible(FiendFolio.ITEM.COLLECTIBLE.PINHEAD) then
		if isPinheadUseSuccessful(player) then
			FiendFolio.AddSewn(entity, player, getStackedPinheadDuration(player, secondHandMultiplier))
		end
	end
end
...
function mod:pinheadOnLocustDamage(player, locust, entity, secondHandMultiplier)
	if locust.SubType == FiendFolio.ITEM.COLLECTIBLE.PINHEAD then
		if math.random() * 8 <= 2 + player.Luck * 0.4 then
			FiendFolio.AddSewn(entity, player, 120 * secondHandMultiplier)
		end
	end
end