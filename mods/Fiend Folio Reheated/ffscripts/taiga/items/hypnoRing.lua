...
local function isHypnoRingUseSuccessful(player)
	local chance = (5 * player:GetCollectibleNum(mod.ITEM.COLLECTIBLE.HYPNO_RING)) + math.min(player.Luck, 10) / 3
...
local function getStackedHypnoRingDuration(player, secondHandMultiplier)
	...
	local result = math.ceil(base * (math.log(math.max(1, player:GetCollectibleNum(mod.ITEM.COLLECTIBLE.HYPNO_RING)), 10) + 1))
...
function mod:hypnoRingOnFireTear(player, tear, secondHandMultiplier)
	if player:HasCollectible(FiendFolio.ITEM.COLLECTIBLE.HYPNO_RING) then
		if isHypnoRingUseSuccessful(player) then
			...
			data.ApplyDrowsySleepDuration = getStackedHypnoRingDuration(player, secondHandMultiplier)
...
function mod:hypnoRingOnKnifeDamage(player, entity, secondHandMultiplier, hasProccedSleep)
	if player:HasCollectible(FiendFolio.ITEM.COLLECTIBLE.HYPNO_RING) and not hasProccedSleep then
		if isHypnoRingUseSuccessful(player) then
			FiendFolio.AddDrowsy(entity, player, 60, getStackedHypnoRingDuration(player, secondHandMultiplier))
		end
	end
end
