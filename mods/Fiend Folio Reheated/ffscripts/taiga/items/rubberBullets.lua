...
local function isRubberBulletsUseSuccessful(player)
	local chance = math.min(12.5, (5 * player:GetCollectibleNum(mod.ITEM.COLLECTIBLE.RUBBER_BULLETS)) + player.Luck * 0.75)
...
local function getStackedRubberBulletsDuration(player, secondHandMultiplier)
	local base = 120 
	local result = math.ceil(base * (math.log(math.max(1, player:GetCollectibleNum(mod.ITEM.COLLECTIBLE.RUBBER_BULLETS)), 5) + 1))
...
function mod:rubberBulletsOnFireTear(player, tear, secondHandMultiplier)
	if player:HasCollectible(FiendFolio.ITEM.COLLECTIBLE.RUBBER_BULLETS) then
		if isRubberBulletsUseSuccessful(player) then
			...
			data.ApplyBruiseDuration = getStackedRubberBulletsDuration(player, secondHandMultiplier)
			...
			if not tear:HasTearFlags(BitSet128(0, 1 << (127 - 64))) then
				...
				sfx:Play(mod.Sounds.ShotgunBlast,0.4,0,false,math.random(60,80)/100)
			end
		end
	end
end
...
function mod:rubberBulletsOnKnifeDamage(player, entity, secondHandMultiplier, hasAppliedBruise)
	if player:HasCollectible(FiendFolio.ITEM.COLLECTIBLE.RUBBER_BULLETS) and not hasAppliedBruise then
		if math.random() * 25 <= math.min(12.5, 5 + player.Luck * 0.75) then
			FiendFolio.AddBruise(entity, player, getStackedRubberBulletsDuration(player, secondHandMultiplier), 1, 1)
			return true
		end
	end
	return false
...
function mod:rubberBulletsOnLocustDamage(player, locust, entity, secondHandMultiplier, hasAppliedBruise)
	...
	if locust.SubType == FiendFolio.ITEM.COLLECTIBLE.NYX then
		if math.random() * 750 <= math.min(12.5, 5 + player.Luck * 0.75) then
			FiendFolio.AddBruise(entity, player, 120 * secondHandMultiplier, 1, 1)
			return true
		end
	end
