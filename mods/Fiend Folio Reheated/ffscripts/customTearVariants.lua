...
function mod:brickTearRender(tear, offset)
	...
	if not tear.Child then
		local brickGfx
		brickGfx = Isaac.Spawn(5, 350, TrinketType.TRINKET_BRICK_ROCK + tear.SubType, tear.Position, nilvector, tear):ToPickup()
...
local function ludoTearHasDirection(tear)
	local player = nil
	if not tear.Parent then
		return false
	elseif tear.Parent:ToPlayer() then
		player = tear.Parent:ToPlayer()
	elseif tear.Parent:ToFamiliar() and tear.Parent:ToFamiliar().Player then
	...
	return player:GetFireDirection() ~= Direction.NO_DIRECTION
end
...
function mod:rocketTearRender(tear, offset)
	...
	elseif flags & TearFlags.TEAR_LUDOVICO == TearFlags.TEAR_LUDOVICO and ludoTearHasDirection(tear) then
		animSuffix = "Move"
...
function mod:hornCobPillTearSplat(tear)
	...
	local smoke = Isaac.Spawn(1000, mod.FF.FFWhiteSmoke.Var, mod.FF.FFWhiteSmoke.Sub, tear.Position, tear.Velocity:Resized(3), tear)
