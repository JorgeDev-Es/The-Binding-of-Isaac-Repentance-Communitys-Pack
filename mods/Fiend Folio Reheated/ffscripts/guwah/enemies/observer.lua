...
function mod:ObserverAI(npc, sprite, data)
    ...
    if not data.Init then
        ...
        tracer.SpriteOffset = Vector(0,-5)
...
function mod:CustomTracerUpdate(effect, sprite, data)
    ...
    local effpos = effect.Position + effect.SpriteOffset
    mod:spritePlay(sprite, "Start")
    effect.SpriteScale = Vector(0.7,0.7)
    data.BeamLength = effpos:Distance(effect.TargetPosition)
end

function mod:CustomTracerRender(effect, sprite, data, isPaused, isReflected)
    if not isReflected then
        local effpos = effect.Position + effect.SpriteOffset
        local angle = mod:GetAngleDegreesButGood(effpos - effect.TargetPosition)
        ...
        tracerbeam:Render(Isaac.WorldToScreen(effpos))
        sprite:Render(Isaac.WorldToScreen(effect.TargetPosition - effect.SpriteOffset))
    end
end