...
function mod:SkulltistAI(npc, sprite, data)
    ...
    if sprite:IsEventTriggered("Shoot") then
        ...
        local skulltistFilter = function(position, candidate)
            if candidate:GetData()[filtertag] and mod:isFriend(candidate) == mod:isFriend(npc) and candidate.EntityCollisionClass > EntityCollisionClass.ENTCOLL_NONE then
                return true
            else
                return false
            end
...
function mod:DoSkulltistDeath(npc, enemy)
    ...
    if enemy:GetData().SkulltistGoEasyOnMe then
        victim:TakeDamage(victim.MaxHitPoints,0,EntityRef(nil),0)
        victim.Velocity = RandomVector()
        victim:BloodExplode()
    else
...
function mod:SkulltistCaseChecks(enemy, victim)
    if victim.Type == mod.FF.Temptress.ID and victim.Variant == mod.FF.Temptress.Var then
        victim:GetData().Suffix = enemy:GetData().Suffix
        victim:GetSprite().FlipX = enemy:GetSprite().FlipX
    end
end
...
function mod:SkulltistWhitelistCheck(npc) 
    for _, entry in pairs(FiendFolio.SkulltistWhitelist) do
        if mod:CheckID(npc, entry) then
            ...
            if entry[6] then
                npc:GetData().SkulltistGoEasyOnMe = true
            end
            return true
        end
    end
