...
function mod:KnightTargetCheck(npc, targetpos, suffix, flipX, margin, range, tightBounds)
    if suffix then
        ...
        local isAligned
        if tightBounds then
            if suffix == "Down" or suffix == "Up" then
                isAligned = math.abs(npc.Position.X - targetpos.X) < margin 
            else
                isAligned = math.abs(npc.Position.Y - targetpos.Y) < margin
            end
        else
            isAligned = (math.abs(npc.Position.X - targetpos.X) < margin or math.abs(npc.Position.Y - targetpos.Y) < margin)
        end
    
        if isAligned and targetpos:Distance(npc.Position) <= range then
            local isBehind = true
            if suffix == "Hori" then
                if flipX then
...
function mod:SuffixToVec(suffix, flipX)
    if suffix == "Up" then
        return Vector(0,-1)
    elseif suffix == "Down" then
        return Vector(0,1)
