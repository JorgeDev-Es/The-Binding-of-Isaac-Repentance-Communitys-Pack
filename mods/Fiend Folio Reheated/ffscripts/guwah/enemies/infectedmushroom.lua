...
function mod:InfectedMushroomAI(npc, sprite, data, isPaused, isReflected)
    ...
    if not data.Init then
        ...
        npc.SplatColor = mod.ColorArcanePink
...
function mod:InfectedConfuzzleUpdate(npc, data)
    ...
    if data.InfectedConfuzzle then
        data.InfectedCooldown = data.InfectedCooldown or mod:RandomInt(30,60,rng)
...
function mod:InfectedGrowthUpdate(effect, sprite, data)
    ...
    if effect.Parent then
        if mod:IsReallyDead(effect.Parent) or effect.Parent:GetData().State == "Death" then
            sfx:Play(SoundEffect.SOUND_DEATH_BURST_SMALL, 0.5)
            local poof = Isaac.Spawn(1000, EffectVariant.BLOOD_EXPLOSION, 1, effect.Position, Vector.Zero, effect):ToEffect()
            poof.Color = mod.ColorArcanePinkA
            poof:Update()
            local splat = Isaac.Spawn(1000, EffectVariant.BLOOD_SPLAT, 0, effect.Position, Vector.Zero, effect):ToEffect()
            splat.Scale = 0.5
            splat.Color = mod.ColorArcanePinkA
            splat:Update()
            effect:Remove()
        end
    end
...
function mod:InfectedRingUpdate(effect, sprite, data)
    ...
    if not sprite:IsPlaying("unappear") then
        if mod:IsReallyDead(effect.Parent) or effect.Parent:GetData().State == "Death" then
