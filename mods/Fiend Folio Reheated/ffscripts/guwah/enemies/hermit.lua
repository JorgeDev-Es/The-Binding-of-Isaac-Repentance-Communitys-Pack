...
function mod:GetHermitCollectible(npc)
    local savedItem = FiendFolio.getFieldInit(FiendFolio.savedata, 'run', 'level', 'HermitData', {})

    if savedItem and savedItem.ID then
        return savedItem.ID
    end
    ...
    local item = itemPool:GetCollectible(ItemPoolType.POOL_TREASURE, true, Isaac.GetPlayer():GetCollectibleRNG(mod.ITEM.COLLECTIBLE.RISKS_REWARD):Next())
    savedItem.ID = item
    return item
end
