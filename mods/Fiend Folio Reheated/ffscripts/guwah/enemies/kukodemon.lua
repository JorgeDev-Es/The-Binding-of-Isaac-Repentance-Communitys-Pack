...
local function FireSplitProjectile(parentProj, angle)
    local proj = Isaac.Spawn(9,0,0,parentProj.Position,Vector(10,0):Rotated(angle),parentProj):ToProjectile()
    -- Inherit CANT_HIT_PLAYER and/or HIT_ENEMIES flags from parent projectile, if present.
    proj.ProjectileFlags = parentProj.ProjectileFlags & (ProjectileFlags.CANT_HIT_PLAYER | ProjectileFlags.HIT_ENEMIES)
end

function mod:KukodemonProjectile(projectile, data)
    if projectile.FrameCount == 30 then
        for i = 0, 270, 90 do
            FireSplitProjectile(projectile, i)
        end
