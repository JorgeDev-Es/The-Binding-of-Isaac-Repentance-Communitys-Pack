...
function mod:BlareAI(npc, sprite, data)
    ...
    elseif data.State == "MonsterShoot" then
        ...
        elseif sprite:GetOverlayFrame() == 10 and not data.Shooted then
            ...
            params.Scale = 1.5
            params.Spread = 0.8
            npc:FireProjectiles(npc.Position, (targetpos - npc.Position):Resized(12.5), 1, params)
    ...
    if data.Transformed then
        ...
        if data.Path then
            if game:GetRoom():CheckLine(npc.Position,targetpos,0,1,false,false) or mod:isScare(npc) then
                npc.Velocity = mod:Lerp(npc.Velocity, mod:reverseIfFear(npc, (targetpos - npc.Position):Resized(data.Speed)), 0.05)
            else
                FiendFolio.FollowPath(npc, data.Speed, data.Path, true, 0.05, 500, true)
            end
        end
    ...
    if not data.Provoked then
        ...
        for _, blare in pairs(Isaac.FindByType(mod.FF.Blare.ID, mod.FF.Blare.Var)) do
            if blare:GetData().WakeUpTimer then
                blare:GetData().WakeUpTimer = blare:GetData().WakeUpTimer / 2
            end
        end
    end
end
