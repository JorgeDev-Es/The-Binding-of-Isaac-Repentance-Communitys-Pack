...
function mod:IDPDGruntAI(npc, sprite, data)
    ...
    if not data.Init then
        ...
        npc:AddEntityFlags(EntityFlag.FLAG_NO_BLOOD_SPLASH | EntityFlag.FLAG_NO_FLASH_ON_DAMAGE)
...
function mod:SpawnIDPD(spawner, spawnpos)
    ...
    if var == mod.FF.IDPDGrunt.Var then
        ...
        for i = 0, 180, 180 do
            ...
            local popo = Isaac.Spawn(mod.FFID.GuwahJoke, var, 0, spawnpos + vec:Rotated(angle):Resized(15), vec:Rotated(angle), spawner):ToNPC()
        ...
        local popo = Isaac.Spawn(mod.FFID.GuwahJoke, var, 0, spawnpos, RandomVector() * 5, spawner)
    end
end 
...
function mod:IDPDPortal(effect, sprite, data)
    ...
    elseif data.State == "Appear" then
        ...
        elseif sprite:IsEventTriggered("Spawn") then --SPAWN THE POPO
            mod:SpawnIDPD(effect, effect.Position)
        else
