...
function mod:DogrockAI(npc, sprite, data)
    ...
    --[[if mod.DogrockIntensity > 0 then
        if Game():GetFrameCount() % 10 == 0 then
            game:MakeShockwave(npc.Position, 0.05 * mod.DogrockIntensity, 0.01 * mod.DogrockIntensity, 120)
        end
    end]]
end
...
function mod:DogrockLogic()
    ...
    if mod.DogrockIntensity > 0 then
        ...
        if mod.CurrentFloorColor then
            ...
            if mod.DogrockIntensity <= 0 then
                if mod.CurrentFloorColor.R >= 0.95 and mod.CurrentFloorColor.G >= 0.95 and mod.CurrentFloorColor.B >= 0.95 then
                    room:SetFloorColor(Color.Default)
                    room:SetWallColor(Color.Default)
                    mod.CurrentFloorColor = nil
                end
            end
        end
