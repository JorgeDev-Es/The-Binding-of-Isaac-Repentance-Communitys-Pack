...
function mod:ResidentAI(npc, sprite, d)
    ...
    elseif d.state == "walking" then
        ...
        if distanceabs < distanceclose and room:CheckLine(npc.Position,targetpos,0,3,false,false) and not mod:isScare(npc)  then
            ...
            elseif room:CheckLine(npc.Position,targetpos,0,gridthreshold,false,ignoreRocks) or d.noBody then
...
function mod:BobsRottenBlast(spawner, spawnpos, blastDmg)
    spawnpos = spawnpos or spawner.Position
    blastDmg = blastDmg or 100
    game:BombExplosionEffects(spawner.Position, blastDmg, 0, mod.ColorBobsGreen, spawner, 1, true, true, DamageFlag.DAMAGE_EXPLOSION)
    local effect1 = Isaac.Spawn(1000, 2, 4, spawner.Position, Vector.Zero, spawner)
    effect1.Color = mod.ColorBobsGreen
    local effect2 = Isaac.Spawn(1000, 7, 0, spawner.Position, Vector.Zero, spawner)
...
function mod:BobsRottenHeadDeath(projectile, data)
    mod:BobsRottenBlast(projectile)
