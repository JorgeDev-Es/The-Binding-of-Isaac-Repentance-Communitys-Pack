...
FiendFolio.ExcelsiorBlacklist = {
    [FiendFolio.ITEM.COLLECTIBLE.ANGELIC_LYRE_B] = true,
    [FiendFolio.ITEM.COLLECTIBLE.ANGELIC_LYRE_R] = true,
    [FiendFolio.ITEM.COLLECTIBLE.ANGELIC_LYRE_Y] = true,
}
    ...
    for _, fireworks in pairs(data.ExcelsiorQueuedUpdates) do
        ...
        if fireworks.FrameCount > 0 then
            if not fireworks.Init then 
                if player:GetActiveCharge(fireworks.UseSlot) + player:GetBatteryCharge(fireworks.UseSlot) < fireworks.PrevCharge or not player:HasCollectible(fireworks.PrevItem) then --This prevents cheese via items which you hold over your head, like the Candle
...
function mod:ExcelsiorActiveLogic(player, data, itemID, useFlags, slot)
	local excelsiorPower = player:GetCollectibleNum(CollectibleType.COLLECTIBLE_EXCELSIOR) + (data.ffsavedata.excelsiorBonus or 0)
    if excelsiorPower > 0 and (useFlags & UseFlag.USE_OWNED > 0) and not FiendFolio.ExcelsiorBlacklist[itemID] then
        data.ExcelsiorQueuedUpdates = data.ExcelsiorQueuedUpdates or {}
