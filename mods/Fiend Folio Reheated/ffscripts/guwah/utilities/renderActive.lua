...
local function ShouldRenderExcelsiorSparkles(player)
	return player:HasCollectible(mod.ITEM.COLLECTIBLE.EXCELSIOR) or (player:GetData().ffsavedata.excelsiorBonus or 0) > 0
end
...
local function GetActiveSlots(player)
    local slots = {}
    if ShouldRenderExcelsiorSparkles(player) then
        for i = 0, 3 do
...
function mod:UpdatePlayerActivePoses()
    ...
    for i = 1, #players do
        ...
        if ShouldRenderExcelsiorSparkles(player)
            and ActivePlayers[i].Player == nil
        then
            AddActivePlayers(i, player)
        elseif not ShouldRenderExcelsiorSparkles(player)
...
function mod:ExcelsiorRender(sprite)
    ...
    for i = 1, #ActivePlayers do
        ...
        then
            ...
            local slots = GetActiveSlots(activePlayer.Player)
            for j = 1, #slots do
