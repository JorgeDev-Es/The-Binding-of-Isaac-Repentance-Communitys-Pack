...
--[[CustomHealthAPI.Library.RegisterRedHealth("TEST_HEART_4", 
                                  {MaxHP = 4,
                                   AnimationFilenames = {EMPTY_HEART = "gfx/ui/test_heart_4.anm2",
                                                         BONE_HEART = "gfx/ui/test_heart_4.anm2",
                                                         LEAKY_HEART = "gfx/ui/test_heart_4.anm2"},								  
                                   AnimationNames = {EMPTY_HEART = {"Fourth", "Third", "Half", "Full"},
                                                     BONE_HEART = {"Fourth", "Third", "Half", "Full"},
                                                     LEAKY_HEART = {"Fourth", "Third", "Half", "Full"}},
                                   SortOrder = 201, 
                                   AddPriority = 201,
                                   HealFlashRO = 128/255, 
                                   HealFlashGO = 0/255, 
                                   ...
                                   ProtectsDealChance = false,
                                   PrioritizeHealing = false,
                                   PickupEntities = {{ID = EntityType.ENTITY_PICKUP, Var = 10, Sub = 1}, 
                                                     {ID = EntityType.ENTITY_PICKUP, Var = 10, Sub = 1}, 
                                                     {ID = EntityType.ENTITY_PICKUP, Var = 10, Sub = 1}, 
                                                     {ID = EntityType.ENTITY_PICKUP, Var = 10, Sub = 1}},
                                   SumptoriumSubType = 666,
                                   SumptoriumSplatColor = morbidSplatColor,
                                   SumptoriumTrailColor = morbidTrailColor,
                                   SumptoriumCollectSoundSettings = {ID = SoundEffect.SOUND_ROTTEN_HEART,
                                                                     Volume = 1.0,
...
--[[CustomHealthAPI.Library.RegisterSoulHealth("TEST_HEART_4_2", 
                                  {MaxHP = 4,
                                   AnimationFilename = "gfx/ui/test_heart_4.anm2",								  
                                   AnimationName = {"Fourth", "Third", "Half", "Full"},
                                   SortOrder = 101, 
                                   AddPriority = 151,
                                   HealFlashRO = 128/255, 
                                   HealFlashGO = 0/255, 
...
function mod:magnetoChaseCheck(pickup, params)
	local closestPlayer
	...
	mod.AnyPlayerDo(function(player)
		...
		or player:HasTrinket(TrinketType.TRINKET_SUPER_MAGNET) then
			...
			if params then
				for k = 1, #params do
					if CustomHealthAPI.Library.CanPickKey(player, params[k]) then
						canChase = true
						break
					end
				end
	...
	if closestPlayer then
		...
		pickup.Velocity = mod:Lerp(pickup.Velocity, vec, 0.2)
...
mod:AddCallback(ModCallbacks.MC_POST_PICKUP_UPDATE, function(_, pickup)
	...
	mod:magnetoChaseCheck(pickup, {"IMMORAL_HEART"})
...
mod:AddPriorityCallback(ModCallbacks.MC_PRE_PICKUP_COLLISION, CustomHealthAPI.Enums.CallbackPriorities.LATE, function(_, pickup, collider)
	if collider.Type == EntityType.ENTITY_PLAYER then
		local collider = collider:ToPlayer()
		local sprite = pickup:GetSprite()
		
		if collider:GetPlayerType() == PlayerType.PLAYER_THESOUL_B and CustomHealthAPI.Helper.IsHoldingTaintedForgotten(collider) then
			return FiendFolio.ImmoralCollisionCallback(_, pickup, collider:GetOtherTwin())
		elseif pickup:IsShopItem() and 
		       (pickup.Price > collider:GetNumCoins() or 
			    (not collider:IsExtraAnimationFinished() and not data.CHAPIHeartPickupSpentSpikesCostAlready))
		then
			return true
		...
		elseif sprite:WasEventTriggered("DropSound") or sprite:IsPlaying("Idle") then
			local redHealthBefore = collider:GetHearts()
			local soulHealthBefore = collider:GetSoulHearts()
...
---@diagnostic disable-next-line: param-type-mismatch
			...
			local redHealthAfter = collider:GetHearts()
			local soulHealthAfter = collider:GetSoulHearts()
			
			if pickup.SubType ~= 1 then
				CustomHealthAPI.Library.AddCandyHeartBonus(collider, redHealthAfter - redHealthBefore, pickup.InitSeed)
				CustomHealthAPI.Library.AddSoulLocketBonus(collider, soulHealthAfter - soulHealthBefore, pickup.InitSeed)
			end
...
mod:AddPriorityCallback(ModCallbacks.MC_PRE_PICKUP_COLLISION, CustomHealthAPI.Enums.CallbackPriorities.LATE, function(_, pickup, collider)
	if collider.Type == EntityType.ENTITY_PLAYER then
		...
		if collider:GetPlayerType() == PlayerType.PLAYER_THESOUL_B and CustomHealthAPI.Helper.IsHoldingTaintedForgotten(collider) then
			return FiendFolio.HalfImmoralCollisionCallback(_, pickup, collider:GetOtherTwin())
		elseif pickup:IsShopItem() and 
		       (pickup.Price > collider:GetNumCoins() or 
			    (not collider:IsExtraAnimationFinished() and not data.CHAPIHeartPickupSpentSpikesCostAlready))
		then
...
mod:AddCallback(ModCallbacks.MC_POST_PICKUP_UPDATE, function(_, pickup)
	mod:keeperFlyCheck(pickup)
	mod:magnetoChaseCheck(pickup, {"RED_HEART", "IMMORAL_HEART"})
	local sprite = pickup:GetSprite()
	if sprite:IsFinished("Appear") then
		sprite:Play("Idle", false)
	end
...
mod:AddPriorityCallback(ModCallbacks.MC_PRE_PICKUP_COLLISION, CustomHealthAPI.Enums.CallbackPriorities.LATE, function(_, pickup, collider)
	if collider.Type == EntityType.ENTITY_PLAYER then
		...
		if collider:GetPlayerType() == PlayerType.PLAYER_THESOUL_B and CustomHealthAPI.Helper.IsHoldingTaintedForgotten(collider) then
			return FiendFolio.BlendedImmoralCollisionCallback(_, pickup, collider:GetOtherTwin())
		elseif pickup:IsShopItem() and 
		       (pickup.Price > collider:GetNumCoins() or 
			    (not collider:IsExtraAnimationFinished() and not data.CHAPIHeartPickupSpentSpikesCostAlready))
		then
...
---@diagnostic disable-next-line: param-type-mismatch
			...
			then
				...
				if CustomHealthAPI.Helper.CanPickKey(collider, "RED_HEART") then
					...
					local hpToSpend = CustomHealthAPI.Library.GetRedHPToBeSpent(collider, hp)
					if hpToSpend > 0 then
						CustomHealthAPI.Library.AddHealth(collider, "RED_HEART", hpToSpend, true)
						sfx:Play(SoundEffect.SOUND_BOSS2_BUBBLES, 1, 0, false, 1.0)
						hp = hp - hpToSpend
					end
					if redIsDoubled then
						hp = math.floor(hp / 2)
				...
				if CustomHealthAPI.Helper.CanPickKey(collider, "IMMORAL_HEART") and hp > 0 then
					CustomHealthAPI.Library.AddHealth(collider, "IMMORAL_HEART", hp, true)
					sfx:Play(mod.Sounds.FiendHeartPickup, 1, 0, false, 1)
				end
			else
				return pickup:IsShopItem()
...
mod:AddCallback(ModCallbacks.MC_POST_PICKUP_UPDATE, function(_, pickup)
	mod:keeperFlyCheck(pickup)
	mod:magnetoChaseCheck(pickup, {"MORBID_HEART"})
	local sprite = pickup:GetSprite()
	if sprite:IsFinished("Appear") then
		sprite:Play("Idle", false)
	end
...
mod:AddPriorityCallback(ModCallbacks.MC_PRE_PICKUP_COLLISION, CustomHealthAPI.Enums.CallbackPriorities.LATE, function(_, pickup, collider)
	if collider.Type == EntityType.ENTITY_PLAYER then
		...
		if collider:GetPlayerType() == PlayerType.PLAYER_THESOUL_B and CustomHealthAPI.Helper.IsHoldingTaintedForgotten(collider) then
			return FiendFolio.MorbidCollisionCallback(_, pickup, collider:GetOtherTwin())
		elseif pickup:IsShopItem() and 
		       (pickup.Price > collider:GetNumCoins() or 
			    (not collider:IsExtraAnimationFinished() and not data.CHAPIHeartPickupSpentSpikesCostAlready))
		then
...
mod:AddPriorityCallback(ModCallbacks.MC_PRE_PICKUP_COLLISION, CustomHealthAPI.Enums.CallbackPriorities.LATE, function(_, pickup, collider)
	if collider.Type == EntityType.ENTITY_PLAYER then
		...
		if collider:GetPlayerType() == PlayerType.PLAYER_THESOUL_B and CustomHealthAPI.Helper.IsHoldingTaintedForgotten(collider) then
			return FiendFolio.TwoThirdsMorbidCollisionCallback(_, pickup, collider:GetOtherTwin())
		elseif pickup:IsShopItem() and 
		       (pickup.Price > collider:GetNumCoins() or 
			    (not collider:IsExtraAnimationFinished() and not data.CHAPIHeartPickupSpentSpikesCostAlready))
		then
...
mod:AddPriorityCallback(ModCallbacks.MC_PRE_PICKUP_COLLISION, CustomHealthAPI.Enums.CallbackPriorities.LATE, function(_, pickup, collider)
	if collider.Type == EntityType.ENTITY_PLAYER then
		...
		if collider:GetPlayerType() == PlayerType.PLAYER_THESOUL_B and CustomHealthAPI.Helper.IsHoldingTaintedForgotten(collider) then
			return FiendFolio.ThirdMorbidCollisionCallback(_, pickup, collider:GetOtherTwin())
		elseif pickup:IsShopItem() and 
		       (pickup.Price > collider:GetNumCoins() or 
			    (not collider:IsExtraAnimationFinished() and not data.CHAPIHeartPickupSpentSpikesCostAlready))
		then
...
CustomHealthAPI.Library.AddCallback("FiendFolio", CustomHealthAPI.Enums.Callbacks.POST_HEALTH_DAMAGED, 0, function(player, flags, key, hpDamaged, wasDepleted, wasLastDamaged)
	if mod.IsActiveRoom() and flags & DamageFlag.DAMAGE_FAKE == 0 and flags & DamageFlag.DAMAGE_DEVIL == 0 then
		if key == "IMMORAL_HEART" and wasLastDamaged then
			local hearts = CustomHealthAPI.Library.GetHealthInOrder(player, true, true)
...
mod:AddPriorityCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, CallbackPriority.LATE, function(_, ent, amount, flags, source, countdown)
