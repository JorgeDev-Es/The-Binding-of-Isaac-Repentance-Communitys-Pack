...
function mod:poobottleAI(npc, subt, variant)
    ...
    elseif d.state == "targetspotted" then
        ...
        if npc.SpriteOffset.Y < -70 and npc.Position:Distance(pooppos) < 10 or not room:IsPositionInRoom(pooppos, 0) then
            d.state = "landhostart"
            d.fallvelocity = nil
    ...
    elseif d.state == "impact" then
        ...
        elseif sprite:GetFrame() == 1 then
            ...
            local grident = game:GetRoom():GetGridEntityFromPos(npc.Position)
            if grident and grident.Desc.Type == GridEntityType.GRID_POOP and d.poop.State ~= 1000 then
                d.poop = grident
                local pooType = grident.Desc.Variant
                d.splattedPooType = pooType
                d.splattedPooType2 = mod:CheckForCustomPoop(grident:GetGridIndex())
                if isDrainFly then
                ...
                if pooType == 1 and isDrainFly then
                    grident:ToPoop().ReviveTimer = math.floor((40 + d.randwait) * 2) + 40
                end
                d.zilch = false
            else
                d.poop = nil
