...
function mod.FindClosestEnemy(pos,radius, nofriends, useCollRad, ignoreSeed, collClass, prioritiseFlagless, ignoreNoHealth, ignoreNoBerserk, ignoreFrozen)
	radius = radius or 99999
	...
	for index,entity in ipairs(Isaac.FindInRadius(pos, radius2, EntityPartition.ENEMY)) do
		if entity:IsVulnerableEnemy() and
		   ...
		   ((not collClass) or (collClass and entity.EntityCollisionClass >= collClass)) and
		   ((not ignoreNoHealth) or entity.HitPoints > 0.0) and
		   ((not ignoreNoBerserk) or (not mod:isBerserkBlacklisted(entity))) and
		   ((not mod:isFriend(entity))) and
		   ((not ignoreFrozen) or (ignoreFrozen and (not entity:HasEntityFlags(EntityFlag.FLAG_FREEZE))))
		then
			local distance = pos:Distance(entity.Position)
			if useCollRad then
...
function mod.FindRandomEnemy(pos, radius, ignoreFriendly)
	pos = pos or Game:GetRoom():GetCenterPos()
	radius = radius or 1000
	local targets = {}
	local target = nil
	for _, entity in pairs(Isaac.FindInRadius(pos, radius, EntityPartition.ENEMY)) do
		if entity and not (ignoreFriendly and entity:HasEntityFlags(EntityFlag.FLAG_FRIENDLY)) and not entity:HasEntityFlags(EntityFlag.FLAG_NO_TARGET) then
			local etype = entity.Type
			local evar = entity.Variant
			local esub = entity.SubType
			local badent = false
...
function mod.FindClosestEntityStarving(npc)
	...
	for _,entity in ipairs(Isaac.GetRoomEntities()) do
		...
		if path:HasPathToPos(entity.Position, false) 
		and entity:IsActiveEnemy() 
		and entity.MaxHitPoints < 100 
		and (not entity:IsBoss()) 
		and (not npc:HasEntityFlags(EntityFlag.FLAG_NO_TARGET) 
		and (not mod:isStatusCorpse(entity))) 
		and entity.EntityCollisionClass > EntityCollisionClass.ENTCOLL_NONE then
			local etype = entity.Type
			local evar = entity.Variant
	...
	elseif entity:IsActiveEnemy() and entity:CanShutDoors() then
		if not framecountboolean or (framecountboolean and entity.FrameCount) > 120 then
			if not entity:HasEntityFlags(EntityFlag.FLAG_FRIENDLY) then
				...
				if entity.Type == mod.FF.SkitterSkull.ID and entity.Variant == mod.FF.SkitterSkull.Var then
					if entity:GetData().State == "Wait" then
						return true
					end
				end
				if entity.Type == mod.FF.Temptress.ID and entity.Variant == mod.FF.Temptress.Var then
					if entity:GetData().State == "Hiding" then
						return true
					end
				end
...
function mod.SpawnGunpowder(npc, pos, timeout, burntime, spreaddist, uniqueColor, dontDoReplace, ashColor, uniqueSpritesheet, friendlyOnly)
	local var = 26
	if npc and npc.Type == 1 or friendlyOnly then
		var = 45
	end
	...
	ash:GetData().friendlyOnly = friendlyOnly
	ash:Update()
end
...
function mod:FindRandomFreePos(npc, radius, findnear, avoidplayer, avoidfire, maxrad)
radius = radius or 0
local validPositions = {}
local validPositionsFar = {}
local validPositionsNear = {}
		...
		if room:GetGridCollisionAtPos(gridpos) == GridCollisionClass.COLLISION_NONE and room:IsPositionInRoom(gridpos, 0) then
			...
			then
				...
				if npc.Position:Distance(gridpos)> radius then
					if maxrad then
						if npc.Position:Distance(gridpos) < maxrad then
							table.insert(validPositionsFar, gridpos)
						end
					else
						table.insert(validPositionsFar, gridpos)
...
function mod:DestroyNearbyGrid(npc, radius, spawnPit, pos)
radius = radius or 30
local validPositions = {}
local pos = pos or npc.Position
local room = game:GetRoom()
local size = room:GetGridSize()
local successfulPit
	for i=0, size do
...
function mod:diagonalMoveFam(fam, speed, returnVec, xmult)
	xmult = xmult or 1
	local ymult = 2 - xmult
	local xvel = speed * xmult
	local yvel = speed * ymult
	if fam.Velocity.X < 0 then
		xvel = xvel * -1
	end
	if fam.Velocity.Y < 0 then
		yvel = yvel * -1
	end
...
function mod:turnGridtoProjectile(spawner, index, velocity, mode, flags, leaveGrid, specialSelection)
	...
	if grid and (grid.CollisionClass == GridCollisionClass.COLLISION_SOLID or grid.CollisionClass == GridCollisionClass.COLLISION_OBJECT or grid.CollisionClass == GridCollisionClass.COLLISION_WALL) then
		...
		if results then
			...
			if gridType == GridEntityType.GRID_ROCK_ALT and gridConfig and gridAnm2 == "gfx/grid/grid_rock.anm2" then
				local roomgfx = mod:getCurrentRoomGfx()
				local backdropType = room:GetBackdropType()
				if roomgfx and roomgfx.Grids and roomgfx.Grids.Rocks then
					pSprite:ReplaceSpritesheet(0, roomgfx.Grids.Rocks)
