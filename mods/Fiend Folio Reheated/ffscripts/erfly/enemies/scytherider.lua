...
function mod:scytheRiderAI(npc)
	...
	local isFriend = mod:isFriend(npc)
	...
	if d.state == "idle" then
		...
		if npc.StateFrame > 30 and d.scythestate == 1 and math.random(30) == 1 and not mod:isScareOrConfuse(npc) and not (isFriend and target:ToPlayer()) then
			local rider = isPitchy and mod.FF.PitchforkHitcher or mod.FF.ScytheRider
			local weapon = isPitchy and mod.FF.HitcherPitchfork or mod.FF.RiderScythe

			local numRiders = 0

			for _, ent in pairs(Isaac.FindByType(rider.ID, rider.Var, -1, true, false)) do
				if mod:isFriend(ent) == isFriend then
					numRiders = numRiders + 1
				end
			end

			local maxThrown = numRiders * 2
			local numThrown = 0

			for _, ent in pairs(Isaac.FindByType(weapon.ID, weapon.Var, -1, true, false)) do
				if mod:isFriend(ent) == isFriend then
					if isPitchy and isFriend and numThrown == maxThrown-1 then
						-- Allow friendly pitchfork hitchers to kill an old pitchfork and throw a new one.
						ent:Kill()
					else
	...
	elseif d.state == "throwscythe" then
		...
		elseif sprite:IsEventTriggered("Shoot") then
			...
			local scythe = Isaac.Spawn(mod.FF.RiderScythe.ID, scythevar, 0, npc.Position + playershoot:Resized(15), playershoot, npc):ToNPC()
			scythe:ClearEntityFlags(EntityFlag.FLAG_APPEAR)
			if mod:isFriend(npc) then
				scythe:AddCharmed(EntityRef(Isaac.GetPlayer()), -1)
				scythe.EntityCollisionClass = EntityCollisionClass.ENTCOLL_ENEMIES
			else
