...
function mod:mrHorfAI(npc, subt, variant)
	...
	if variant == mod.FF.MrRedHorf.Var then
		...
		speeds = {6, 0.8}
	elseif variant == mod.FF.MrGurgle.Var then
		headchoice = mod.FF.MrGurgleHead.Var
		speeds = {3.5, 0.5}
	...
	if d.state == "idle" then
		if npc.Velocity:Length() > 0.1 then
			if variant == mod.FF.MrGurgle.Var then
	...
	elseif d.state == "throw" then
		if sprite:IsFinished("HeadThrow") then
			local newid = 11
			local newvar = 1
			if npc.Variant == mod.FF.MrGurgle.Var then
				newid = EntityType.ENTITY_SPLASHER
				newvar = 0
			elseif r:RandomInt(3) == 1 then
				newvar = 0
			end
			npc:Morph(newid, newvar, variant, -1)
		...
		elseif sprite:IsEventTriggered("Shoot") then
			...
			npc:PlaySound(SoundEffect.SOUND_SHELLGAME,0.8,0,false,0.8)

			if npc.Variant == mod.FF.MrSubHorf.Var then
				local vec = (targetpos - npc.Position):Resized(12)
				local head = Isaac.Spawn(EntityType.ENTITY_SUB_HORF, 0, 0, npc.Position + vec:Resized(8), vec, npc):ToNPC();
				head:ClearEntityFlags(EntityFlag.FLAG_APPEAR)
				head.State = 3
				head:Update()
				npc.Velocity = vec:Resized(8)
				mod:FlipSprite(sprite, npc.Position, npc.Position + vec)
				d.bounding = true
			else
				...
				local clamppos = room:GetClampedPosition(target.Position + RandomVector()*math.random(radius), -10)
...
function mod:mrHorfHeadAI(npc, subt, variant)
	local sprite = npc:GetSprite()
	local d = npc:GetData()

	local endresult = {12, 0}
	local airtime = 15
	if variant == mod.FF.MrRedHorfHead.Var then
		endresult = {mod.FF.RedHorf.ID, mod.FF.RedHorf.Var}
		d.Explosiveness = 95
	elseif variant == mod.FF.MrGurgleHead.Var then
		npc.SplatColor = mod.ColorGurgleGibs
		airtime = 20
	end
	...
	if d.thrown then
		...
		local throwspeed = math.min(dist / airtime, 10)
		...
		if npc.FrameCount > airtime then
			d.thrown = false
		...
		elseif variant == mod.FF.MrGurgleHead.Var and sprite:IsEventTriggered("Shoot") then
			mod:BobsRottenBlast(npc, npc.Position, 20)
...
function mod:mrHorfKill(npc, variant)
    if not (npc:HasEntityFlags(EntityFlag.FLAG_FREEZE) or npc:HasEntityFlags(EntityFlag.FLAG_MIDAS_FREEZE) or mod:isLeavingStatusCorpse(npc)) then
        ...
        if not d.hasthrown then
            ...
            local spawned = Isaac.Spawn(newid, newvar, variant, npc.Position, npc.Velocity, npc)
