...
function mod:brickRockPlayer(player, data)
    if player:HasTrinket(FiendFolio.ITEM.ROCK.BRICK_ROCK) then
        ...
        if mod:canUseDrawnItem(player, mod.DrawnItemTypes.BrickRock, aim) then
			...
			local isSmelted = false
			
			local t1 = player:GetTrinket(1)
			local t0 = player:GetTrinket(0)
			if t1 == FiendFolio.ITEM.ROCK.BRICK_ROCK + TrinketType.TRINKET_GOLDEN_FLAG or
			   t0 == FiendFolio.ITEM.ROCK.BRICK_ROCK + TrinketType.TRINKET_GOLDEN_FLAG
			then
				player:TryRemoveTrinket(FiendFolio.ITEM.ROCK.BRICK_ROCK + TrinketType.TRINKET_GOLDEN_FLAG)
				isGolden = true
			elseif t1 == FiendFolio.ITEM.ROCK.BRICK_ROCK or
			       t0 == FiendFolio.ITEM.ROCK.BRICK_ROCK
			then
				player:TryRemoveTrinket(FiendFolio.ITEM.ROCK.BRICK_ROCK)
			else
				-- smelted trinkets oh boy oh boy oh boy
				local multiSub = 0
				...
				if multiBefore == multiAfter then
					player:TryRemoveTrinket(FiendFolio.ITEM.ROCK.BRICK_ROCK)
					
					multiAfter = player:GetTrinketMultiplier(FiendFolio.ITEM.ROCK.BRICK_ROCK)
					multiAfter = math.max(0, multiAfter - multiSub)
				end
				
				isGolden = (multiBefore - multiAfter) == 2
				isSmelted = true
			end
...
mod:AddCallback(ModCallbacks.MC_POST_PICKUP_UPDATE, function(_, trinket)
	if mod:GetRealTrinketId(trinket.SubType) == FiendFolio.ITEM.ROCK.BRICK_ROCK and trinket:GetData().BrickFollowParent then
...
mod:AddCallback(ModCallbacks.MC_PRE_PICKUP_COLLISION, function(_, trinket, collider, low)
	if mod:GetRealTrinketId(trinket.SubType) == FiendFolio.ITEM.ROCK.BRICK_ROCK and
