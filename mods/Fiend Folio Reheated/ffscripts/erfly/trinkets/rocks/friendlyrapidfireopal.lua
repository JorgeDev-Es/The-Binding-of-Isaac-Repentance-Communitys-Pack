	...
	if d.rapidFireOpalAmmo then
        ...
        if (#d.rapidFireOpalGun == 0) or (d.rapidFireOpalGun and (not d.rapidFireOpalGun[1]:Exists())) then
            for i = 1, math.max(gunCount, #d.rapidFireOpalGun) do
                if not d.rapidFireOpalGun[i] or (d.rapidFireOpalGun[i] and not d.rapidFireOpalGun[i]:Exists()) then
                    ...
                    d.rapidFireOpalGun[i]:GetData().PosOffset = mod.rapidFireOpalOffsets[gunCount][1] + (mod.rapidFireOpalOffsets[gunCount][2] * (i-1))
                    gun.Parent = player
                end
            end
        end
        ...
        if mod:canUseDrawnItem(player, mod.DrawnItemTypes.RapidFireOpal, aim) then
            for i = 1, math.max(gunCount, #d.rapidFireOpalGun) do
                ...
                local tear = player:FireTear(player.Position + aim:Resized(10), Vector(player.ShotSpeed * 10,0):Rotated(aim:GetAngleDegrees()):Rotated(mod.rapidFireOpalOffsets[gunCount][1] + (mod.rapidFireOpalOffsets[gunCount][2] * (i-1))))
