...
function mod:middleHandPlayerUpdate(player, data)
    if player:HasTrinket(TrinketType.TRINKET_LEFT_HAND) and player:HasTrinket(mod.ITEM.TRINKET.RIGHT_HAND) then
        if player:IsExtraAnimationFinished() then
            local eitherGulped
            local makeGolden
            local t0 = mod:GetRealTrinketId(player:GetTrinket(0))
            local t1 = mod:GetRealTrinketId(player:GetTrinket(1))
            ...
            if t1 == TrinketType.TRINKET_LEFT_HAND and t0 ~= mod.ITEM.TRINKET.RIGHT_HAND
            or t0 == TrinketType.TRINKET_LEFT_HAND and t1 ~= mod.ITEM.TRINKET.RIGHT_HAND 
            or t1 == mod.ITEM.TRINKET.RIGHT_HAND and t0 ~= TrinketType.TRINKET_LEFT_HAND 
            or t0 == mod.ITEM.TRINKET.RIGHT_HAND and t1 ~= TrinketType.TRINKET_LEFT_HAND 
            then
                eitherGulped = true
            end
            if makeGolden then
...
function mod:getClosetSpecificTrinket(pos, trinket)
    ...
    for _, v in pairs(Isaac.FindByType(5,350,-1, false, false)) do
        if mod:GetRealTrinketId(v.SubType) == trinket then
...
mod:AddCallback(ModCallbacks.MC_POST_PICKUP_UPDATE, function(_, pickup)
    if mod:GetRealTrinketId(pickup.SubType) == TrinketType.TRINKET_LEFT_HAND then
    ...
    elseif mod:GetRealTrinketId(pickup.SubType) == mod.ITEM.TRINKET.RIGHT_HAND then
...
mod:AddCallback(ModCallbacks.MC_PRE_PICKUP_COLLISION, function(_, pickup, collider)
    if collider.Type == 5 and collider.Variant == 350 then
        if mod:GetRealTrinketId(pickup.SubType) == TrinketType.TRINKET_LEFT_HAND and mod:GetRealTrinketId(collider.SubType) == mod.ITEM.TRINKET.RIGHT_HAND then
