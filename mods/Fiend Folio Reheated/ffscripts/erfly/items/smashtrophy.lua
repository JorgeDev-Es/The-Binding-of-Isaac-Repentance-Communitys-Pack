...
function mod:smashTrophyEntityUpdate(npc, data)
    if data.smashedByTrophy then
        if data.smashedByTrophy.Vec then
            local mult = (1 - (math.max(npc.HitPoints,1)/math.max(npc.MaxHitPoints,1)))
            mult = math.max(mult, 0) --For enemies with more hitpoints than maxhitpoints
            local smashvec = data.smashedByTrophy.Vec:Resized(10 * data.smashedByTrophy.Timer * mult)
