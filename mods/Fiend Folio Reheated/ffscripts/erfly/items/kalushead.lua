    ...
    if aim:Length() > 0.5 and player:GetActiveCharge(d.holdingFFItemSlot) > 0 then
                    ...
                    if not enemy:HasEntityFlags(EntityFlag.FLAG_NO_STATUS_EFFECTS) then
                        if enemy:ToNPC() then
                            ...
                            elseif not enemy:ToNPC():IsBoss() then
                                --Makes it 1.5ish, no integers. I bet there's some way to exploit this
                                local slowness = 1 + game:GetFrameCount() % 2
        ...
        if d.kaluHasBeenUsed then
            ...
            if player:HasTrinket(mod.ITEM.ROCK.ELECTRUM) then
                ...
                mod:alternateElectrumShock(player, rng, player.Damage*mult, player.Position, 3)
            end
        end 
    end
end
