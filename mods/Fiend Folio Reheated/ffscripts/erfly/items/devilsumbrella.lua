...
function mod:firePiss(player, fireVec)
	...
	splat:Update()
	local spurts = math.ceil(10 * (math.log(math.max(1, player:GetCollectibleNum(mod.ITEM.COLLECTIBLE.DEVILS_UMBRELLA)), 3) + 1))
	for i = 1, spurts do
...
function mod:devilsUmbrellaPostFire(player, tear, rng, pdata, tdata, ignorePlayerEffects, isLudo)
	--Devil's Umbrella
	if player:HasCollectible(mod.ITEM.COLLECTIBLE.DEVILS_UMBRELLA) and
