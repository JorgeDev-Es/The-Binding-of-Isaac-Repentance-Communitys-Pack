...
function mod:grapplingHookAI(e)
	...
	if d.state == "flying" then
        ...
        local room = game:GetRoom()
        local checkMode = room:GetGridCollisionAtPos(e.Position + e.Velocity) >= 4 and 2 or room:GetGridCollisionAtPos(e.Position) >= 4 and 1
		if checkMode then
            if hookmode == 1 then
			    d.state = "hooked"
    ...
    elseif d.state == "reelin" then
        ...
        if player:HasTrinket(mod.ITEM.ROCK.ELECTRUM) and not d.electrumShocked then
            local mult = mod.GetGolemTrinketPower(player, FiendFolio.ITEM.ROCK.ELECTRUM)
            local rng = player:GetCollectibleRNG(mod.ITEM.COLLECTIBLE.GRAPPLING_HOOK)
            mod:alternateElectrumShock(player, rng, player.Damage*mult, e.Position, 2)
            d.electrumShocked = true
        end
