...
mod:AddCallback(ModCallbacks.MC_USE_PILL, function(_, pillEffect, player, flags)
    ...
    pdata.CustomHealthAPIPersistent = pdata.CustomHealthAPIPersistent or {}
    local pillNum = pdata.CustomHealthAPIPersistent.CurrentlyHeldPill or player:GetPill(0)
    ...
    if pillEffect == PillEffect.PILLEFFECT_CYANIDE then
        ...
        if (pillNum == 961 or pillNum == 3009) and not (flags & UseFlag.USE_NOHUD > 0 or flags & UseFlag.USE_MIMIC > 0) then
            if math.random(10) ~= 1 then
    ...
    if sdata and sdata.Cyanide then
        if sdata.Cyanide.Timer and sdata.Cyanide.Strength then
            ...
            if sdata.Cyanide.Timer <= 0 then
                ...
                --I wonder if someone could glitch it due to the frame somehow
                mod.scheduleForUpdate(function()
                    player:Kill()
                end, 1, nil, true)
            end
...
function mod:cyanideRoomClear(spawnPos, dropRNG)
    ...
    if mod.savedata.run and mod.savedata.run.TryingToSpawnCyanide then
        ...
        if rand < mod.savedata.run.TryingToSpawnCyanide then
            if mod.anyPlayerHas(CollectibleType.COLLECTIBLE_STARTER_DECK) then
                local cardSpawns = {
                    mod.ITEM.CARD.ACE_OF_CUPS, mod.ITEM.CARD.TWO_OF_CUPS, mod.ITEM.CARD.THREE_OF_CUPS,
                }
                local card = Isaac.Spawn(5, 300, cardSpawns[dropRNG:RandomInt(#cardSpawns) + 1], spawnPos, nilvector, nil)
            else
