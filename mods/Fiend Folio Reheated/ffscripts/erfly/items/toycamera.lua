...
function mod:toyCameraFlashUpdate(e)
    ...
    if e.FrameCount >= 5 then
        ...
        elseif sprite:IsEventTriggered("AddToCringeCollection") then
                    ...
                    if (testPos.X <= e.Position.X + 48 + entityColl) and (testPos.X >= e.Position.X - 48 - entityColl) and (testPos.Y <= e.Position.Y + 32 + entityColl) and (testPos.Y >= e.Position.Y - 32 - entityColl) then
                        if entity:ToNPC() then
                            if entity.Type == 33 then
                                entity:Die()
                            elseif (entity.Type == mod.FF.WhispersController.ID and entity.Variant == mod.FF.WhispersController.Var) then
                                local secret = Isaac.Spawn(mod.FF.SecretGhost.ID, mod.FF.SecretGhost.Var, mod.FF.SecretGhost.Sub, entity.Position, nilvector, nil):ToEffect()
                                secret:FollowParent(entity)
                                secret:Update()
            ...
            if e.SpawnerEntity:ToPlayer() and e.SpawnerEntity:ToPlayer():HasTrinket(mod.ITEM.ROCK.ELECTRUM) then
                ...
                mod:alternateElectrumShock(player, rng, player.Damage*mult, e.Position, #insideEntities)
            end
            ...
            elseif insideCount == 0 and math.random(100000) == 1 then
            ...
            for i = 1, 5 do
                mod.scheduleForUpdate(function()
                    for _, entity in pairs(Isaac.GetRoomEntities()) do
                        if (entity.Type == EntityType.ENTITY_PROJECTILE and not entity:ToProjectile():HasProjectileFlags(ProjectileFlags.CANT_HIT_PLAYER))
                        or (entity.Type == 1000 and mod.EffectsToyCameraCanDelete[entity.Variant])
                        or (entity.Type == mod.FFID.Tech and mod.TechnicalEntsToyCameraCanDelete[entity.Variant])
                        then
                            local vec = entity.Position - e.Position
                            vec = vec:Rotated(-sprite.Rotation)
                            local testPos = e.Position + vec
...
mod:AddCallback(ModCallbacks.MC_USE_CARD, function(_, cardID, player, useFlags)
    ...
    FiendFolio:trySayAnnouncerLine(mod.Sounds.VAObjectCoolPhoto, useFlags, 45)
end, Card.COOL_PHOTO)

function mod:secretGhostAI(e)
    local sprite, d = e:GetSprite(), e:GetData()
    if not d.init then
        d.init = true
        local sprite = e:GetSprite()
        sprite:ReplaceSpritesheet(0, "gfx/bosses/whispers/ghosty4.png")
        sprite:LoadGraphics()
        e.Color = Color(1,1,1,0.5)
    end
    mod:spritePlay(sprite, "Idle")
    e.Color = Color(e.Color.R, e.Color.G, e.Color.B, e.Color.A - 0.01, e.Color.RO, e.Color.BO, e.Color.GO)
    if e.Color.A < 0 then
        e:Remove()
    end
end