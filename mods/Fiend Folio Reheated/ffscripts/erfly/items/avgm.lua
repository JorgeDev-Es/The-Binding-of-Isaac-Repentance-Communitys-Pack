...
function mod:incrementAVGM(player, spawnEnt, isLocust)
    ...
    if isLocust then
        ...
        local carBatteryCount = player:GetCollectibleNum(CollectibleType.COLLECTIBLE_CAR_BATTERY)
        local clicks = 1 + carBatteryCount
        if (d.AVGMoveralluses % (clicks * 2)) > (clicks - 1) then
            game:Darken(1,1000)
		...
		if d.AVGMuses >= d.AVGMnext then
            ...
            if not isLocust then
                ...
                if player:HasTrinket(mod.ITEM.ROCK.ELECTRUM) then
                    ...
                    mod:alternateElectrumShock(player, rng, player.Damage*mult*3, player.Position, 1)
                end
            end
