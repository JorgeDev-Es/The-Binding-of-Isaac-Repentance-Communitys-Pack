    ...
    if player:HasCollectible(FiendFolio.ITEM.COLLECTIBLE.INFINITY_VOLT) then
        ...
        elseif (not data.infinityVoltFired) then
            if aim:Length() > 0.5 then
                ...
                if playerType == PlayerType.PLAYER_BLACKJUDAS or playerType == PlayerType.PLAYER_JUDAS_B then
                    local sprite = plug:GetSprite()
                    sprite:ReplaceSpritesheet(0, "gfx/effects/effect_infinityvolt_shadow.png")
                    sprite:LoadGraphics()
                end
                plug:Update()
...
function mod:infinityVoltCordEndUpdate(e)
    ...
    if e.Parent then
        ...
        if not e.Child then
            ...
            local playerType = p:GetPlayerType()
            if playerType == PlayerType.PLAYER_BLACKJUDAS or playerType == PlayerType.PLAYER_JUDAS_B then
                local rsprite = rope:GetSprite()
                rsprite:ReplaceSpritesheet(0, "gfx/effects/effect_infinityvolt_shadow.png")
