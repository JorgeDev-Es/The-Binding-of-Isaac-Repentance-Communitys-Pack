...
function mod:sanguineHookAI(e)
	...
	if d.pullingOut then
		...
		if player:HasTrinket(mod.ITEM.ROCK.ELECTRUM) then
			...
			mod:alternateElectrumShock(player, rng, player.Damage*mult, d.hooktarget.Position, 3, nil, nil, nil, {d.hooktarget})
		end
	...
	elseif d.state == "hooked" then
		d.hookTime = d.hookTime or 0
		d.hookTime = d.hookTime + 1
		...
		if d.hooktarget and d.hooktarget:Exists() and ((d.hooktarget.Type ~= 5 and d.hooktarget.Type ~= 3 and d.hooktarget.EntityCollisionClass >= EntityCollisionClass.ENTCOLL_PLAYEROBJECTS) or ((d.hooktarget.Type == 5 or d.hooktarget.Type == 3) and d.hooktarget.Position:Distance(e.Parent.Position) > e.Parent.Size + d.hooktarget.Size)) then
			...
			if d.hooktarget.Type == 5 then
				if d.hooktarget.TargetPosition then
					if d.hooktarget.Variant == PickupVariant.PICKUP_COIN and d.hooktarget.SubType == CoinSubType.COIN_STICKYNICKEL then
						--unique sticky nickel behaviour
						if d.hookTime > 60 then
							d.hooktarget:ToPickup():Morph(5, 20, CoinSubType.COIN_NICKEL, true, true, true)
							d.hooktarget:GetSprite():Play("Idle")
							d.hooktarget.EntityCollisionClass = EntityCollisionClass.ENTCOLL_ALL
							sfx:Play(mod.Sounds.RipcordPop, 1, 0, false, 1)
						end
					else
