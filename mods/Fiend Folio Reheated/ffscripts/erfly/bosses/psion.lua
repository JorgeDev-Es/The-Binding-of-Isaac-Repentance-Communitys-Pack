	...
	elseif d.projType == "psionlaseyes" then
		...
		if not d.mode then
			if v.FrameCount == 10 then
				...
				local position = EntityLaser.CalculateEndPoint(v.Position, d.playerposition - v.Position, Vector.Zero, v, 0)
				local indicatorBeam = Isaac.Spawn(1000, 175, 0, position, Vector.Zero, v):ToEffect()
				indicatorBeam.Parent = v
				indicatorBeam.Target = indicatorBeam
				indicatorBeam.Color = Color(1, 0, 1, 0.5)
				indicatorBeam.DepthOffset = -1000
				indicatorBeam.Timeout = 10
				indicatorBeam.PositionOffset = Vector(0,-20)
				indicatorBeam:Update()
			end
		elseif d.mode == 1 then
			v.Velocity = nilvector
