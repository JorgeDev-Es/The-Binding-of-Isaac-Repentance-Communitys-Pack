...
mod:AddCallback(ModCallbacks.MC_USE_CARD, function(_, cardID, player, flags)
	...
	for _, pickup in ipairs(Isaac.FindByType(5, 20, -1, false, false)) do
		if pickup.SubType == CoinSubType.COIN_GOLDEN then
			pickup:ToPickup():Morph(5, 20, CoinSubType.COIN_GOLDENCURSEDPENNY, true)
		elseif not FiendFolio.DenyJackofDiamonds[pickup.SubType] then
...
mod:AddCallback(ModCallbacks.MC_USE_CARD, function(_, cardID, player, flags)
	...
	for _, pickup in ipairs(Isaac.FindByType(5, 40, -1, false, false)) do
		if pickup.SubType == BombSubType.BOMB_DOUBLEPACK or pickup.SubType == FiendFolio.PICKUP.BOMB.MIXED_DOUBLE then
			pickup:ToPickup():Morph(5, 40, FiendFolio.PICKUP.BOMB.DOUBLE_COPPER, true)
		elseif not FiendFolio.DenyJackofClubs[pickup.SubType] then
			pickup:ToPickup():Morph(5, 40, FiendFolio.PICKUP.BOMB.COPPER, true)
		end
	end
	for _, pickup in ipairs(Isaac.FindByType(4, -1, -1, false, false)) do
		if pickup.Variant == BombSubType.BOMB_TROLL or pickup.Variant == BombSubType.BOMB_SUPERTROLL then
			if pickup.SubType ~= 160 and pickup.SubType ~= 923 then
				if not pickup:GetSprite():IsPlaying("Explode") then
					Isaac.Spawn(4,3,160,pickup.Position,pickup.Velocity,nil)
					pickup:Remove()
				end
			end
		end
...
mod:AddCallback(ModCallbacks.MC_USE_CARD, function(_, cardID, player, flags)
	...
	for _, pickup in ipairs(Isaac.FindByType(5, 40, -1, false, false)) do
		...
		elseif not FiendFolio.DenyMisprintedJackofClubs[pickup.SubType] then
