...
mod:AddCallback(ModCallbacks.MC_USE_CARD, function(_, cardID, player, useflags)
	...
	if math.random(5) == 1 then
		...
		mod:trySayAnnouncerLine(mod.Sounds.VARuneSoulFiend, useflags)
	end
...
mod:AddCallback(ModCallbacks.MC_USE_CARD, function(_, cardID, player, flags)
	...
	mod:trySayAnnouncerLine(mod.Sounds.VARuneSoulGolem, flags, 40)
...
mod:AddCallback(ModCallbacks.MC_USE_CARD, function(_, cardID, player, useflags)
	...
	if choice == Card.CARD_SOUL_LAZARUS then
		...
		player:UseCard(choice, UseFlag.USE_NOANNOUNCER)
	end
	...
	mod:trySayAnnouncerLine(mod.Sounds.VARuneSoulRandom, useflags, 20)
