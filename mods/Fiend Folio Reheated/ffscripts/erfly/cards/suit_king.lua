...
mod:AddCallback(ModCallbacks.MC_USE_CARD, function(_, cardID, player, flags)
	...
	for _, pickup in ipairs(Isaac.FindByType(5, 350, -1, false, false)) do
		...
		if not mod:IsGoldTrinket(pickup.SubType) then
			successGround = true
	...
	if t0 > 0 then
		if not mod:IsGoldTrinket(t0) then
			player:TryRemoveTrinket(t0)
			player:AddTrinket(t0 + TrinketType.TRINKET_GOLDEN_FLAG)
			successInv = true
		end
	end
	if t1 > 0 then
		if not mod:IsGoldTrinket(t1) then
			player:TryRemoveTrinket(t1)
			player:AddTrinket(t1 + TrinketType.TRINKET_GOLDEN_FLAG)
			successInv = true
		end
	end
