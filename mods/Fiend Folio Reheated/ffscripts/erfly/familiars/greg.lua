...
function mod.randomArrayWeightBased(chosenArray, rng)
	local totalWeight = 0
	for i = 1, #chosenArray do
		totalWeight = totalWeight + chosenArray[i].Weight
	end

	local rand = (rng and rng:RandomFloat() or math.random()) * totalWeight
...
mod:AddCallback(ModCallbacks.MC_PRE_FAMILIAR_COLLISION, function(_, fam, collider)
    if collider:ToProjectile() then
		if collider:ToProjectile():HasProjectileFlags(ProjectileFlags.CANT_HIT_PLAYER) then
			return true
		end
    	collider:Die()
		local d = fam:GetData()
