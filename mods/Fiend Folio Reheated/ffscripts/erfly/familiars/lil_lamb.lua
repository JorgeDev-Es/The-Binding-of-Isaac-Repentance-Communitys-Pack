...
function mod:lilLambAI(familiar)
	...
	local playerMarked = player:HasCollectible(CollectibleType.COLLECTIBLE_MARKED)
	...
	elseif d.state == "idle" then
		...
		local shouldCharge
		if (targetPos or playerMarked) and d.chargeCount > d.chargeTimer then
			shouldCharge = false
		...
		elseif d.chargeCount and d.chargeCount > d.chargeTimer then
			...
			if isSirenCharmed then
				...
				elseif playerMarked and aimDir and aimDir ~= Direction.NO_DIRECTION then
					tear.Velocity = player:GetAimDirection():Resized(10)
				end
