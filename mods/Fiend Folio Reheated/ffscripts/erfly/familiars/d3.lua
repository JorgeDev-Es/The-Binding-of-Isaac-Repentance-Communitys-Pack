...
mod:AddCallback(ModCallbacks.MC_FAMILIAR_UPDATE, function(_, fam)
    ...
    for _, tear in ipairs(Isaac.FindInRadius(fam.Position, 15, EntityPartition.TEAR)) do
        ...
        local denied
        if td.specialfiend or td.implosionCard then
            denied = true
        end
        if not (td.hasBeenD3Rolled or denied) then
            ...
            for i = 1, math.random(count) do
                if math.random(3) == 1 then
                    ...
                    tear.TearFlags = tear.TearFlags | mod:SetTearFlag(math.random(TearFlags.TEAR_EFFECT_COUNT) - 1)
                end
            end
            ...
            if not bd.hasBeenD3Rolled then
                if bomb.Position:Distance(fam.Position) < 15 then
                    for i = 1, d3tears[math.random(#d3tears)] do
                        bomb:AddTearFlags(mod:SetTearFlag(math.random(TearFlags.TEAR_EFFECT_COUNT) - 1))
                    end
                ...
                if not ld.shortenedByD3 then
                    ...
                    for i = 1, d3tears[math.random(#d3tears)] do
                        if math.random(3) == 1 then
                            ...
                            wackyLaser.TearFlags = wackyLaser.TearFlags | mod:SetTearFlag(math.random(TearFlags.TEAR_EFFECT_COUNT) - 1)
                        end
                    end
                    local setColor
                    if math.random(3) == 1 then
