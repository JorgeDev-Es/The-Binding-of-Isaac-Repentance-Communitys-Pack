...
function mod:catchTheseHandsPlayerHurt(player, amount)
	if Isaac.GetChallenge() == mod.challenges.catchTheseHands then
		...
		--NEW HAND
		if amount > 1 then
			player:AddTrinket(mod.ITEM.TRINKET.REDHAND + TrinketType.TRINKET_GOLDEN_FLAG, nil, true)
		else
			player:AddTrinket(mod.ITEM.TRINKET.REDHAND, nil, true)
		end
