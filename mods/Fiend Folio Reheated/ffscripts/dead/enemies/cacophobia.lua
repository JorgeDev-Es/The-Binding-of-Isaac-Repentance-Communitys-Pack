...
local function clearCacophobiaEnts(data)
    ...
    for _, entity in ipairs(Isaac.FindByType(mod.FF.Bubble.ID, mod.FF.Bubble.Var)) do
        entity:Remove()
    end
    ...
    for _, collectible in ipairs(Isaac.FindByType(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_COLLECTIBLE, 0)) do -- remove the collectible spawned by vanishing twin once you get the item
        collectible:Remove()
    end
...
function mod:cacophobiaAI(npc, sprite, data)
    if not data.Init then
        local others = Isaac.FindByType(mod.FF.CacophobiaVenus.ID, mod.FF.CacophobiaVenus.Var)
        for _, other in ipairs(others) do
            if other:GetData().Init then
                npc:Remove()
                return
            end
        end
        ...
        data.Renderer:AddEntityFlags(EntityFlag.FLAG_DONT_OVERWRITE)
    ...
    if not data.Blackout then
        ...
        if data.Blackout <= 0 then
            ...
            for anim, option in pairs(idleAnimations) do
                if (not option.Condition or option.Condition(npc, data)) and (not FiendFolio.ReducedFrights or option.ReducedFrights) then
            ...
            if mod.activeJevilEffects and mod.activeJevilEffects[0] then
                sfx:Play(mod.Sounds.EENEHitWacky, 1, 0, false, 0.9, 0)
            else
            ...
            if not noPlayMusic then
                if not FiendFolio.ReducedFrights then
        ...
        if data.SetPlayerPosition then
            ...
            local darkEsaus = Isaac.FindByType(EntityType.ENTITY_DARK_ESAU)
            local awayFromVenus = npc.Position + (npc.Position - data.SetPlayerPosition):Resized(300)
            for _, esau in ipairs(darkEsaus) do
                esau.Position = awayFromVenus
            end
        end
    end
    ...
    if data.Cooldown <= 0 and not data.Blackout then
        ...
        if newPattern.BlackoutTime ~= 0 then
            if mod.activeJevilEffects and mod.activeJevilEffects[0] then
                sfx:Play(mod.Sounds.EENEHit, 1, 0, false, 1, 0)
            else
        ...
        if sprite:GetAnimation() == "Death" then
            ...
            if sprite:IsEventTriggered("The Field") then
                if not data.TheField then
                    if mod.activeJevilEffects and mod.activeJevilEffects[0] then
                        sfx:Play(mod.Sounds.EENEHitEnd, 1, 0, false, 1, 0)
                    else
                        sfx:Play(mod.Sounds.VenusBackToFight, 1, 0, false, 0.9, 0)
                    end
                    music:Resume()
            ...
            elseif sprite:IsEventTriggered("Blackout") then
                if data.TheField then
                    if mod.activeJevilEffects and mod.activeJevilEffects[0] then
                        sfx:Play(mod.Sounds.EENEHit, 0.2, 0, false, 1, 0)
                    else
                        sfx:Play(mod.Sounds.VenusToBlack, 1, 0, false, 1, 0)
                    end
                    music:Pause()
            ...
            local darkEsaus = Isaac.FindByType(EntityType.ENTITY_DARK_ESAU)
            for _, esau in ipairs(darkEsaus) do
                esau.Position = room:GetCenterPos() + Vector(0, -300)
            end
...
mod:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, function(_, entity, amount, flags, source, cooldown)
    if source.Type == mod.FFID.Boss or source.Type == 9 or source.Type == mod.FFID.Boss or mod.FF.Hitbox.ID then
        local srcIsCaco = false
        if source.Type == mod.FFID.Boss and source.Variant == mod.FF.CacophobiaVenus.Var then
            srcIsCaco = true
        else
            if source.Entity and source.Entity.SpawnerEntity then source = source.Entity.SpawnerEntity end
            if source.Type == mod.FFID.Boss and source.Variant == mod.FF.CacophobiaVenus.Var then
                srcIsCaco = true
            end
        end
        if srcIsCaco then
            if mod.WillDamageBeFatal(entity:ToPlayer(), amount, flags, true, true) then
                entity:GetData().cacoDeathState = 0     -- init
            end
        end
    end
...
mod:AddCallback(ModCallbacks.MC_POST_PLAYER_UPDATE, function(_, player)
    if player:GetData().cacoDeathState then
        if player:IsDead() then
            player:GetData().cacoDeathState = 1     -- dying
        else
            if player:GetData().cacoDeathState == 1 then        -- died but revived
                player:GetData().cacoDeathState = false
            end
        end
    end
end)
...
mod:AddCallback(ModCallbacks.MC_POST_GAME_END, function (_, died)
    if died then
        ...
        for _, p in pairs(players) do
            if p:GetData().cacoDeathState then
                p:GetData().cacoDeathState = 2          -- triggers screen render
                return
            end
        end
    end
...
StageAPI.AddCallback("FiendFolio", "POST_HUD_RENDER", 1, function(isPauseMenuOpen, pauseMenuDarkPct)
    local players = Isaac.FindByType(1, -1, -1, false, false)
    for _, p in pairs(players) do
        if p:GetData().cacoDeathState == 2 then
            local screen = Sprite()
            screen:Load("gfx/bosses/cacophobia/death screen.anm2", true)
            screen:Play("screen", true)
            screen:Render(StageAPI.GetScreenCenterPosition(), Vector.Zero, Vector.Zero)
        end
    end
end)