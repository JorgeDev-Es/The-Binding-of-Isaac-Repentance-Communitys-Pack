...
function mod.rubberRockSpawn(customGrid)
    ...
    if grid.State ~= 2 then
        ...
        local stage, stagetype = game:GetLevel():GetStage(), game:GetLevel():GetStageType()
        local skin
        if stage == LevelStage.STAGE3_1 
        or stage == LevelStage.STAGE3_2 
        or ((stage == LevelStage.STAGE2_1 or stage == LevelStage.STAGE2_2) and stagetype == StageType.STAGETYPE_AFTERBIRTH)
        or (stage == LevelStage.STAGE6 and stagetype == StageType.STAGETYPE_ORIGINAL) then
            skin = "gfx/grid/grid_rubber_rock_chap3.png"
        elseif stage == LevelStage.STAGE2_1 or stage == LevelStage.STAGE2_2 then
            skin = "gfx/grid/grid_rubber_rock_chap2.png"
        end
        if skin then
