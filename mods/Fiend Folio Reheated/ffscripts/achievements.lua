FiendFolio.ACHIEVEMENT = {
	...
	},
	{
		...
		Tooltip = {"pick up", "1000 pickups", "in total"},
		Tags = {"Misc"},
	},
	{
		ID = "VENDING_MACHINE",
		Note = "vending_machine",
		Name = "vending machines",
		Tooltip = {"use five", "different", "active items", "in a run"},
		Tags = {"Misc"},
	},
	{
		ID = "TOO_MANY_OPTIONS",
		Note = "too_many_options",
		Name = "too many options",
		Tooltip = {"enter an", "ultra secret", "room"},
		Item = {
...
FiendFolio.AchievementTrackerIds = achievementTrackerIds
...
mod:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, function()
	if mod.INITIALISED_UNLOCKS then
		...
		if not FiendFolio.ACHIEVEMENT.TOO_MANY_OPTIONS:IsUnlocked(true) and room:GetType() == RoomType.ROOM_ULTRASECRET then
			FiendFolio.ACHIEVEMENT.TOO_MANY_OPTIONS:Unlock()
		end
...
-- thirteen of stars
mod:AddCallback(ModCallbacks.MC_USE_ITEM, function(_, id, rng, player, flags)
	...
	if id > 0 and not FiendFolio.ACHIEVEMENT.VENDING_MACHINE:IsUnlocked(true) then
		if flags == flags | UseFlag.USE_OWNED or flags == flags | UseFlag.USE_VOID then
			mod.savedata.run.uniqueActivesUsed = mod.savedata.run.uniqueActivesUsed or {}
			local uniqueActive = true
			for i = 1, #mod.savedata.run.uniqueActivesUsed do
				if id == mod.savedata.run.uniqueActivesUsed[i] then
					uniqueActive = false
					break
				end
			end
			if uniqueActive then
				table.insert(mod.savedata.run.uniqueActivesUsed, id)
				if #mod.savedata.run.uniqueActivesUsed >= 5 then
					FiendFolio.ACHIEVEMENT.VENDING_MACHINE:Unlock()
				end
			end
			--print(#mod.savedata.run.uniqueActivesUsed)
		end
	end
end)
...
-- Token
mod:AddCallback(ModCallbacks.MC_POST_PICKUP_INIT, function(_, pickup)
	if not mod.ACHIEVEMENT["TOKEN"]:IsUnlocked() then
		pickup:Morph(5, PickupVariant.PICKUP_COIN, CoinSubType.COIN_PENNY)
	end
end, mod.PICKUP.VARIANT.TOKEN)
