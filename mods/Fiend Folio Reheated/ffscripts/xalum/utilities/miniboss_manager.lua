...
local function MakeChapter5Miniboss(roomsList, rng)
	...
	overwriteDesc.DisplayFlags = mod.GetExpectedMinibossDisplayFlags()
	overwriteDesc.SurpriseMiniboss = true
	overwriteDesc.Flags = 0
...
mod:AddCallback(ModCallbacks.MC_POST_NEW_LEVEL, function()
	...
	if level:GetStage() == LevelStage.STAGE5 and not (BasementRenovator and BasementRenovator.Loaded) then
		local rng = RNG()
...
mod:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, function()
	...
	if room:GetType() == RoomType.ROOM_MINIBOSS then
		...
		if hud then --This is an insanely stupid check
			mod.IsFFMiniboss = true
		end
	end
end)
