...
function mod.GetCompletionNoteLayerDataFromPlayerType(playerType)
	for key, dataset in pairs(mod.savedata.completion) do
		if playertype_cache[key] == playerType then
			local paperOffset = 0
			if dataset.istainted then
				paperOffset = REPENTANCE_PLUS and 5 or 3
			end
			return {
				[noteLayer.DELI] 	= dataset.deli + paperOffset,
...
-- Sets the modded character completion marks tracked by REPENTOGON (and shown in the main menu) to the ones tracked by FF.
-- This gets called when the save file is loaded + whenever FF saves its savedata.
function mod:SyncCompletionMarksWithRepentogon(savedata)
	if not REPENTOGON or not savedata or not savedata.completion then return end

	-- The `playertype_cache` might not be initialized yet (such as on first save file load), so start from FF's PlayerTypes.
	for _, playerType in pairs(mod.PLAYER) do
		local playerConf = EntityConfig.GetPlayer(playerType)
		if playerConf then
			-- Create the key that this character's completion would have been saved under.
			-- Will be the same as the key created in `mod.InitCharacterCompletion()`.
			local lookup = string.lower(playerConf:GetName())
			if playerConf:IsTainted() then lookup = lookup .. "b" end

			local marks = savedata.completion[lookup]
			if marks then
				Isaac.SetCompletionMarks({
					PlayerType = playerType,
					MomsHeart = marks.heart,
					Isaac = marks.isaac,
					Satan = marks.satan,
					BossRush = marks.rush,
					BlueBaby = marks.bbaby,
					Lamb = marks.lamb,
					MegaSatan = marks.mega,
					UltraGreed = marks.greed,
					Hush = marks.hush,
					Delirium = marks.deli,
					Mother = marks.mother,
					Beast = marks.beast,
...
-- On save file select, if REPENTOGON is installed, sync FF's character completion marks with the ones REPENTOGON shows in the main menu.
if REPENTOGON then
	mod:AddCallback(ModCallbacks.MC_POST_SAVESLOT_LOAD, function(_, saveSlot, isSlotSelected)
		if isSlotSelected and mod:HasData() then
			-- Read directly from the json because FF itself doesn't load its save data until run start.
			local savedata = json.decode(mod:LoadData())
			mod:SyncCompletionMarksWithRepentogon(savedata)
		end
	end)
end
