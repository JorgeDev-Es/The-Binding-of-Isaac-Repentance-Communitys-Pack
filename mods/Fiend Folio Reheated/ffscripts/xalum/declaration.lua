...
local function shouldGetNewTargetPosition(entity, gridDistanceOverride)
	local data = entity:GetData()
	local room = game:GetRoom()

	return (
		not data.targetGridPosition or
		data.targetGridPosition:Distance(entity.Position) < (gridDistanceOverride or 5) or
...
-- Version 2.0!
function mod.XalumGridPathfind(entity, targetPosition, speedLimit, lerpPercent, gridDistanceOverride)
	local data = entity:GetData()

	if shouldGetNewTargetPosition(entity, gridDistanceOverride) then
		local room = game:GetRoom()
	...
	if data.targetGridPosition then
		...
		entity.Velocity = mod.XalumLerp(entity.Velocity, targetVelocity, lerpPercent or 0.4)
	else
...
function mod.XalumGenericPathfinding(npc, speedLimit, lerpStrengthOverride)
	...
	if FiendFolio:isScareOrConfuse(npc) or (not (data.lastGridCollision and data.lastGridCollision + 15 > npc.FrameCount) and game:GetRoom():CheckLine(npc.Position, targetPosition, 0)) then
		targetPosition = FiendFolio:confusePos(npc, targetPosition)

		local targetVelocity = (targetPosition - npc.Position):Resized(speedLimit)
		targetVelocity = FiendFolio:reverseIfFear(npc, targetVelocity)

		npc.Velocity = mod.XalumLerp(npc.Velocity, targetVelocity, lerpStrengthOverride or 0.2)
	else
...
function mod.Xalum_globinpathfind(npc, speedlimit, target)
	...
	local hasFear = npc:HasEntityFlags(EntityFlag.FLAG_FEAR)
	local hasConfusion = npc:HasEntityFlags(EntityFlag.FLAG_CONFUSION)

	if hasConfusion then
		data.globinLastConfusionDirection = data.globinLastConfusionDirection or RandomVector()

		if npc.FrameCount % 10 == 0 then
			data.globinLastConfusionDirection = RandomVector()
		end

		npc.Velocity = data.globinLastConfusionDirection:Resized(npc.Velocity:Length() + 1)
	else
		if game:GetRoom():CheckLine(npc.Position, target + (npc.Position - target):Resized(5), 0, 1, false, false) and not (data.lastgridcollision and data.lastgridcollision + 15 > npc.FrameCount) then
			if hasFear then
				npc.Velocity = (npc.Position - target):Resized(npc.Velocity:Length() + 1)
			else
				npc.Velocity = (target - npc.Position):Resized(npc.Velocity:Length() + 1)
			end
		elseif npc.Pathfinder:HasPathToPos(target, false) then
			if hasFear then
				local direction = target - npc.Position
				local newTarget = npc.Position + direction:Rotated(180)
				npc.Pathfinder:FindGridPath(newTarget, npc.Velocity:Length() + 0.1, 2, false)
			else
	...
	npc.Velocity = npc.Velocity:Resized(math.min((hasConfusion and speedlimit / 2 or speedlimit), npc.Velocity:Length() * 1.2))
end
...
function mod.XalumFindWall(position, direction)
	...
	until grid and (grid:GetType() == GridEntityType.GRID_WALL or grid:GetType() == GridEntityType.GRID_DOOR) and not StageAPI.IsCustomGrid(room:GetGridIndex(position))
...
function mod.XalumIsPlayerUsingHorsePill(player, flags)
	...
	local pillColour = player:GetData().CustomHealthAPIPersistent.CurrentlyHeldPill or player:GetPill(0)
...
function mod.QuickSetEntityGridPath(entity, valueOverride, force)
	local room = game:GetRoom()
	local positionIndex = room:GetGridIndex(entity.Position)
	valueOverride = valueOverride or 900

	if force or (entity.EntityCollisionClass ~= EntityCollisionClass.ENTCOLL_NONE and room:GetGridPath(positionIndex) <= valueOverride) then
...
function mod.FireClusterProjectiles(npc, velocity, numProjectiles, params)
	...
	return projectiles, corpseClusterParent
end
...
function mod.IsPlayerHoldingTwoTrinkets(player, trinket, strict)
	for i = 0, 1 do
		local held = player:GetTrinket(i)
		if not (held == trinket or (held & ~ TrinketType.TRINKET_GOLDEN_FLAG == trinket and not strict)) then
			return false
		end
	end
