...
mod:AddCallback(ModCallbacks.MC_POST_CURSE_EVAL, function(_, curses)
    ...
    if mod.anyPlayerHas(CollectibleType.COLLECTIBLE_BLACK_LANTERN) then
        ...
        player:GetData().lastBlackLanternCurse = curse
        return curse
    end
end)
...
mod:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, function(_, ent, damage, flags, source, countdown)
    ...
    if levelHasCurse(level, FiendFolio.curses.sunCurse) and (flags & DamageFlag.DAMAGE_NO_PENALTIES == 0) and (flags & DamageFlag.DAMAGE_IV_BAG == 0) and (flags & DamageFlag.DAMAGE_FAKE == 0) then
...
mod:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, function(_, ent, damage, flags, source, countdown)
    ...
    if levelHasCurse(level, FiendFolio.curses.masterCurse) and (flags & DamageFlag.DAMAGE_NO_PENALTIES == 0) and (flags & DamageFlag.DAMAGE_IV_BAG == 0) and (flags & DamageFlag.DAMAGE_FAKE == 0) then
        if sd.masterLevel then
...
--black candle synergy: rain doesnt hurt the player
mod:AddCallback(ModCallbacks.MC_POST_PLAYER_UPDATE, function(_, player)
    local level = game:GetLevel()
...
mod:AddCallback(ModCallbacks.MC_POST_PROJECTILE_UPDATE, function(_, projectile) --ugh!
    if projectile:ToProjectile():GetData().projType == "cursed rain" and projectile:ToProjectile().Height >= -60 then
        if mod.anyPlayerHas(CollectibleType.COLLECTIBLE_BLACK_CANDLE) then
            local players = Isaac.FindInRadius(projectile.Position, 10, EntityPartition.PLAYER)
            if #players > 0 then
                projectile:Kill()
            end
        end
...
mod:AddCallback(ModCallbacks.MC_POST_NEW_LEVEL, function()
   ...
   --reset last curse
   player:GetData().lastBlackLanternCurse = 1 << -1

   if persistBLCurseThisFloor and mod.anyPlayerHas(CollectibleType.COLLECTIBLE_BLACK_CANDLE) then
     local level = game:GetLevel()
     local player = Isaac.GetPlayer(0)
...
mod.AddItemPickupCallback(function(player, added)
    ...
    if player:GetData().lastBlackLanternCurse then
        curse = player:GetData().lastBlackLanternCurse
    else
        curse = getRandomLanternCurse(player)
    end
    level:AddCurse(curse, false)
end, nil, CollectibleType.COLLECTIBLE_BLACK_CANDLE, true)
