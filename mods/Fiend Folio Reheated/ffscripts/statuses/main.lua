...
function mod:handleAnimAndMoveSpeedMultipliers(entity, data, sprite)
	...
	if data.FFDrowsyDuration ~= nil and data.FFDrowsyDuration >= 0 then
		...
		if not mod.hasCollidablePart(entity) then
			statusMultiplier = math.max(statusMultiplier, 0.2)
		end
	end
...
mod:AddCallback(ModCallbacks.MC_POST_UPDATE, function()
	...
	for _, npc in ipairs(npcs) do
		...
		mod:jevilBruseEnemies(npc) -- moving this to after bruise logic so damage actually applies -taiga
