...
local ftrinkets = fitems .. "trinkets."
local fgolem = "ffscripts.ferrium.items.golem."
local fspells = "ffscripts.ferrium.spells."
FiendFolio.LoadScripts({
	...
	fenemy .. "acolyte",
	fenemy .. "edema",
	fenemy .. "forgottenfetus",
	fenemy .. "anode",
	fenemy .. "laideronnette",
	fenemy .. "cubert", --idk if this is counts as a joke
	...
	ftrinkets .. "autopsykit",
	ftrinkets .. "fleacircus",
	ftrinkets .. "spiregrowth",
	ftrinkets .. "sharppenny",
	ftrinkets .. "angryfaic",
	ftrinkets .. "pocketdice",
	ftrinkets .. "lockedshackle",
	ftrinkets .. "bombtoken",
	...
	fgolem .. "hiddenite",
	fgolem .. "pocketrockit",
	fgolem .. "homoeroticruby",
	fgolem .. "gaygarnet",
	fgolem .. "lithopedian",
	fgolem .. "jesusrock",
	fgolem .. "chunkofgallium",
	fgolem .. "onionrock",

	--Spells
	--Am I really doing this for a joke
	fspells .. "main",
	fspells .. "moonbeam",
	...
	fitems .. "d122",
	fitems .. "fiendsthirdleg",
	
	
	--Non Enemy Shenanigans
	"ffscripts.ferrium.misc.cellgame",
	"ffscripts.ferrium.misc.launchedStuff", --code shell for jumping enemies
	"ffscripts.ferrium.misc.rockshadows", --flashing shadows for rock projectiles
	"ffscripts.ferrium.misc.customProjectile", --code shell for custom projectiles
	"ffscripts.ferrium.misc.trinketFamiliarGameInit", --prevents duplicating trinket familiars
	"ffscripts.ferrium.misc.heldActives", --set data when players fire stuff like the candle
	--"ffscripts.ferrium.misc.renderSpells", --rendering for spell slots
})
...
mod.FerriumEffects = {
	...
	[mod.FF.ChorusAfterimage.Sub] = {
		Update = mod.chorusAfterimageEffect,
	},
	[mod.FF.MoonbeamTarget.Sub] = {
		Update = mod.moonbeamTargetEffect,
	},
	[mod.FF.MoonbeamLight.Sub] = {
		Update = mod.moonbeamLightEffect,
	},
	[mod.FF.WomanIndicator.Sub] = {
		Update = mod.goAwayEffect,
		Animation = "Woman"
	},
	[mod.FF.GenericAfterimage.Sub] = {
		Update = mod.genericAfterimageEffect,
	}
}
...
mod.FerriumEnemies = {
	...
	[mod.FF.ForgottenFetus.Var] = {
		Update = mod.forgottenFetusAI,
		Coll = mod.forgottenFetusColl,
	},
	[mod.FF.LilAl.Var] = {
		Update = mod.lilAlAI,
	},
	[mod.FF.ForgottenFetusHitbox.Var] = {
		Update = mod.forgottenFetusHitboxAI,
		Hurt = mod.forgottenFetusHitboxHurt,
	},
	[mod.FF.Anode.Var] = {
		Update = mod.anodeAI,
	},
	...
	[mod.FF.Laideronnette.Var] = {
		Update = mod.laideronnetteAI,
		Hurt = mod.laideronnetteHurt,
	},
	[mod.FF.Cubert.Var] = {
		Update = mod.cubertAI,
	},
...
function mod.ferriumProj(v, d)
	...
	mod.laideronnetteProj(v, d)

	mod.customProjectileBehavior(v, d)
end
...
mod:AddCallback(ModCallbacks.MC_POST_ENTITY_REMOVE, function(_,proj1)
	...
	mod.laideronneteProjRemove(proj, data)
...
function mod.ferriumTear(v, d)
	...
	mod.fiendsThirdLegTear(v, d)
	mod.chubberTear(v)
	mod.pocketRocketTear(v, d)
	mod.dontHitImmediatelyTear(v, d)
	mod:chunkOfGalliumTearUpdate(v, d)

	mod.customTearBehavior(v, d)
end
...
--mod:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, mod.refreshFerriumTables)
...
mod:AddCallback(ModCallbacks.MC_POST_PEFFECT_UPDATE, function(_, player)
	...
	mod:trinketFamiliarGameInit(player, data) --man this updating before ffsavedata does is annoying
	mod:overheadHeldActiveUpdate(player, data)
	...
	mod:fistfulOfAshUpdate(player, data)
	mod:pocketRocketUpdate(player, data)
	mod:electrumUpdate(player, data)
	mod:goldShiLunchUpdate(player, data)
	mod:bombTokenUpdate(player, data)
	mod:jesusRockUpdate(player, data)
	...
	for i=1,tempEffects.Size do
		...
		print(item.ID, effect.Count, effect.Cooldown)
		--print(tempEffs:GetCollectibleEffectNum(326))
	end]]
end)

mod:AddCallback(ModCallbacks.MC_POST_PLAYER_UPDATE, function(_, player)
	local data = player:GetData()
	mod:jesusRockSuperUpdate(player, data)
end)

function mod:ferriumPostTearRemove(tear, data)
	mod:fiendsThirdLegTearRemove(tear, data)
	mod:pocketRocketTearRemove(tear, data)
	mod:electrumTearRemove(tear, data)

	mod.customTearRemove(tear, data)
end

function mod:ferriumPreTearCollision(tear, ent, data)
end

function mod:ferriumPrePlayerCollision(player, collider, low)
	mod:electrumCollision(player, collider, low)
end
...
function mod:ferriumPlayerOnNewRoom()
	...
	for i = 1, game:GetNumPlayers() do
		...
		if data.overheadActiveItemID and data.overheadActiveItemID == CollectibleType.COLLECTIBLE_NOTCHED_AXE then
		else
		...
		mod:electrumNewRoom(player, data)
		mod:lockedShackleNewRoom(player, data)
	end
	...
	mod.scheduleForUpdate(function()
		mod:fistfulOfAshNewRoom()
	end, 1)
	mod.lithopedianNewRoom()
	mod:onionRockNewRoom()
end
...
function mod:ferriumPlayerOnNewLevel()
	for i = 1, game:GetNumPlayers() do
		...
		mod:fistfulOfAshNewLevel(player)
		mod:spellsNewLevel(player, data)
	end
...
function mod:ferriumOnFireTear(player, tear, secondHandMultiplier, isLudo, ignorePlayerEffects)
	...
	mod:fiendsThirdLegOnFireTear(player, tear, secondHandMultiplier)
	...
	mod:pocketRocketOnFireTear(player, tear, isLudo, ignorePlayerEffects)
	mod:homoeroticRubyOnFireTear(player, tear, secondHandMultiplier)
	mod:gayGarnetOnFireTear(player, tear, isLudo, ignorePlayerEffects)
	mod:warmGeodeOnFireTear(player, tear, secondHandMultiplier)
	mod:chunkOfGalliumOnFireTear(player, tear, secondHandMultiplier)
...
function mod:ferriumOnFireLaser(player, laser)
	...
	mod:pocketRocketOnFireLaser(player, laser)
	mod:gayGarnetOnFireLaser(player, laser)
	mod:chunkOfGalliumOnFireLaser(player, laser)
end
...
function mod:ferriumOnFireKnife(player, knife)
	...
	mod:pocketRocketOnFireKnife(player, knife)
	mod:gayGarnetOnFireKnife(player, knife)
	mod:chunkOfGalliumOnFireKnife(player, knife)
end

function mod:ferriumOnKnifeUpdate(player, knife, data)
	mod:chunkOfGalliumKnifeUpdate(player, knife, data)
end

function mod:ferriumOnKnifeReset(player, knife, data)
	mod:chunkOfGalliumKnifeReset(player, knife, data)
end
...
function mod:ferriumOnFireBomb(player, bomb, secondHandMultiplier)
	...
	mod:fiendsThirdLegOnFireBomb(player, bomb, secondHandMultiplier)
	mod:pocketRocketOnFireBomb(player, bomb)
	mod:homoeroticRubyOnFireBomb(player, bomb, secondHandMultiplier)
	mod:gayGarnetOnFireBomb(player, bomb, secondHandMultiplier)
	mod:warmGeodeOnFireBomb(player, bomb)
end

function mod:ferriumPostBombRemove(bomb, data)
	mod:fiendsThirdLegBombRemove(bomb, data)
end

function mod:ferriumPostBombUpdate(bomb, data)
	mod:fiendsThirdLegBombUpdate(bomb, data)
	mod:chunkOfGalliumBombUpdate(bomb, data)
end

function mod:ferriumOnKnifeDamage(player, ent, secondHandMultiplier)
	...
	mod:fiendsThirdLegOnKnifeDamage(player, ent, secondHandMultiplier)
	mod:homoeroticRubyOnKnifeDamage(player, ent, secondHandMultiplier)
	--mod:gayGarnetOnKnifeDamage(player, ent, secondHandMultiplier)
end

function mod:ferriumOnFireRocket(player, target, secondHandMultiplier)
	...
	mod:fiendsThirdLegOnFireRocket(player, target, secondHandMultiplier)
	mod:homoeroticRubyOnFireRocket(player, target, secondHandMultiplier)
	mod:gayGarnetOnFireRocket(player, target, secondHandMultiplier)
	mod:warmGeodeOnFireRocket(player, target, secondHandMultiplier)
	mod:chunkOfGalliumOnFireRocket(player, target, secondHandMultiplier)
end

function mod:ferriumOnLaserDamage(player, ent, secondHandMultiplier)
	...
	mod:fiendsThirdLegOnLaserDamage(player, ent, secondHandMultiplier)
	mod:homoeroticRubyOnLaserDamage(player, ent, secondHandMultiplier)
	--mod:gayGarnetOnLaserDamage(player, ent, secondHandMultiplier)
end

function mod:ferriumOnFireAquarius(player, creep, secondHandMultiplier)
	...
	mod:fiendsThirdLegOnFireAquarius(player, creep, secondHandMultiplier)
	mod:homoeroticRubyOnFireAquarius(player, creep, secondHandMultiplier)
	mod:gayGarnetOnFireAquarius(player, creep, secondHandMultiplier)
	mod:warmGeodeAquarius(player, creep, secondHandMultiplier)
end

function mod:ferriumOnDarkArtsDamage(player, ent, secondHandMultiplier)
	...
	mod:fiendsThirdLegOnDarkArtsDamage(player, ent, secondHandMultiplier)
	mod:homoeroticRubyOnDarkArtsDamage(player, ent, secondHandMultiplier)
	mod:gayGarnetOnDarkArtsDamage(player, ent, secondHandMultiplier)
end

function mod:ferriumGenericNPCUpdate(npc)
	mod:starSapphireChecks(npc)
	mod:angryFaicNPCUpdate(npc)
	mod:fakeKnockbackEffect(npc)
	mod:electrumNPCUpdate(npc)
	mod:homoeroticRubyWomen(npc)
end
mod:AddCallback(ModCallbacks.MC_NPC_UPDATE, mod.ferriumGenericNPCUpdate)


function mod:ferriumGenericDamage(ent, damage, flags, source)
	local returnData = {}
	local tempReturnData = {newDamage = damage}
	if ent and ent:ToNPC() then
		local npc = ent:ToNPC()
		mod:achillesRockEntDamage(npc, tempReturnData, flags, source)
		mod:focusCrystalEntDamage(npc, tempReturnData, flags, source)
		mod:fragmentedOnyxGeodeEntDamage(npc, tempReturnData, flags, source)
		mod:bombTokenEntDamage(npc, tempReturnData, flags, source)
	end

	if tempReturnData.sendNewDamage then
		returnData = mod:makeCopiedTable(tempReturnData)
	end
	return returnData
end
...
mod:AddCallback(ModCallbacks.MC_POST_NPC_RENDER, function(_,npc)
	...
	mod:flyingFuck(npc, data)
	mod:electrumRender(npc, data)
end)
...
function mod:ferriumTrinketUpdate(trinket)
	mod:dadsLegendaryGoldenRockUpdate(trinket)
	mod:dogrockRockTrinketUpdate(trinket)
	mod:fractalGeodeTrinketUpdate(trinket)
	mod:grimlinTrinketUpdate(trinket)
	mod:sandDollarTrinketUpdate(trinket)
	mod:homoeroticRubyTrinketUpdate(trinket)
end
mod:AddCallback(ModCallbacks.MC_POST_PICKUP_UPDATE, mod.ferriumTrinketUpdate, 350)

function mod:ferriumFamiliarCache(player)
	mod:lithopedianFamiliar(player)
	mod:fetalStoneFamiliar(player)
	mod:starSapphireFamiliar(player)
	mod:tigersEyeFamiliar(player)
	mod:sunShardFamiliar(player)
	mod:onionRockFamiliar(player)
end

--[[function mod:printEffects(npc)
	local sprite = npc:GetSprite()
	--fuck off decoration insects
	local blacklist = {[7] = true,[66] = true,[87] = true,[12] = true,[15] = true,[21] = true, [21343] = true, [68] = true, [33] = true, [2] = true, [121] = true}
...
--Modified random valid path function to get random enemies closer to Isaac, or further away. 0 is close, 1 is far. 
	...
	for _,grid in ipairs(mod.GetGridEntities()) do
		if grid.CollisionClass == GridCollisionClass.COLLISION_SOLID then
			...
			if grid:GetType() == GridEntityType.GRID_ROCKT then
				...
				--mod.perfectVermin[grid:GetGridIndex()] = 4
			end
		end
		...
		if proj:IsDead() then
			...
			if room:GetGridCollisionAtPos(proj.Position) == GridCollisionClass.COLLISION_NONE and data.leaveGrid ~= nil and not dontSpawn then
				...
				if data.leaveCustomGrid then
					...
					if gridEnt then
						if data.customAltRock and sprite:GetFilename() == "gfx/grid/grid_rock.anm2" then
			...
			elseif data.specialGridDeath then
				...
				if data.d122Wisp and data.d122Wisp:Exists() then
					local wisp = Isaac.Spawn(3, 206, 604, proj.Position, Vector.Zero, data.d122Wisp):ToFamiliar()
					wisp.Player = data.d122Wisp
					wisp.TargetPosition = proj.Position
					wisp:Update()
				end
			else
				if data.d122Wisp and data.d122Wisp:Exists() then
...
function mod:makeCopiedTable(original)
	...
	for key,entry in pairs(original) do
		if type(entry) == "table" then
			tab[key] = mod:makeCopiedTable(entry)
		else
			tab[key] = entry
		end
	end
...
function mod:superExists(npc)
	if npc and npc:ToNPC() then
		if npc and npc:Exists() and not mod:isStatusCorpse(npc) then
			return true
		else
			return false
		end
...
function mod:check115Hurt(npc, damage, flag, source)
	if mod.FerriumEnemies2[npc.Variant] and mod.FerriumEnemies2[npc.Variant].Hurt then
		return mod.FerriumEnemies2[npc.Variant].Hurt(_,npc, damage, flag, source)
	end
end
mod:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, mod.check115Hurt, mod.FFID.Ferrium2)

function mod:check115Kill(npc)
    if mod.FerriumEnemies2[npc.Variant] and mod.FerriumEnemies2[npc.Variant].Kill then
		mod.FerriumEnemies2[npc.Variant].Kill(_,npc, npc.Variant)
	end
end
mod:AddCallback(ModCallbacks.MC_POST_ENTITY_KILL, mod.check115Kill, mod.FFID.Ferrium2)

function mod:check115Death(npc)
	if mod.FerriumEnemies2[npc.Variant] and mod.FerriumEnemies2[npc.Variant].Death then
		mod.FerriumEnemies2[npc.Variant].Death(_,npc, npc.Variant)
	end
end
mod:AddCallback(ModCallbacks.MC_POST_NPC_DEATH, mod.check115Death, mod.FFID.Ferrium2)

function mod:check115(npc)
	if mod.FerriumEnemies2[npc.Variant] and mod.FerriumEnemies2[npc.Variant].Update then
		mod.FerriumEnemies2[npc.Variant].Update(_, npc, npc.SubType, npc.Variant)
	end
end
mod:AddCallback(ModCallbacks.MC_NPC_UPDATE, mod.check115, mod.FFID.Ferrium2)

function mod:check115Collision(npc, collider, bool)
	if mod.FerriumEnemies2[npc.Variant] and mod.FerriumEnemies2[npc.Variant].Coll then
		return mod.FerriumEnemies2[npc.Variant].Coll(_, npc, collider, bool)
	end
end
mod:AddCallback(ModCallbacks.MC_PRE_NPC_COLLISION, mod.check115Collision, mod.FFID.Ferrium2)

function mod:check115Render(npc, offset)
	if mod.FerriumEnemies2[npc.Variant] and mod.FerriumEnemies2[npc.Variant].Render then
		return mod.FerriumEnemies2[npc.Variant].Render(_, npc, offset)
	end
end
mod:AddCallback(ModCallbacks.MC_POST_NPC_RENDER, mod.check115Render, mod.FFID.Ferrium2)
...
function mod:ferriumPressButtonLogic(player, ci)
	--DEBUG KEY PRESS
	--[[if mod.AnyKeyboardTriggered(Keyboard.KEY_L, ci) then
		local room = game:GetRoom()
		local sapph = Isaac.Spawn(5, 350, mod.ITEM.ROCK.SAPPHIC_SAPPHIRE, room:GetCenterPos()+Vector(-80,-80), Vector.Zero, nil)
		local rub = Isaac.Spawn(5, 350, mod.ITEM.ROCK.HOMOEROTIC_RUBY, room:GetCenterPos()+Vector(80,80), Vector.Zero, nil)
		player:AddCacheFlags(CacheFlag.CACHE_ALL)
		player:EvaluateItems()
	end]]
end

function mod:ferriumCommands(cmd, params)
	if cmd == "reviveme" or cmd == "revive" then
		for i = 1, game:GetNumPlayers() do
			local player = Isaac.GetPlayer(i - 1)
			local isPlayingDeathAnimation = player:GetSprite():GetAnimation():sub(-#"Death") == "Death"
			if isPlayingDeathAnimation then
				player:GetData().pleasegivemethisonechanceitwasunfairdamage = true
				player:GetData().ffsavedata.JesusRockAddedLazEffect = true
...
function mod:findActiveSlot(num, player)
	for i=0,2 do
		if player:GetActiveItem(i) == num then
			return i
		end
	end
end

function mod.dontHitImmediatelyTear(v, d)
	if d.dontHitImmediately then
		if d.dontHitImmediately > 0 then
			d.dontHitImmediately = d.dontHitImmediately-1
			v.EntityCollisionClass = EntityCollisionClass.ENTCOLL_NONE
		else
			v.EntityCollisionClass = EntityCollisionClass.ENTCOLL_ALL
			d.dontHitImmediately = nil
		end
	end
end