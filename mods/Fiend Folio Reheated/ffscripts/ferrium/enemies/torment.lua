...
function mod:tormentAI(npc)
	...
	elseif data.state == "SpawnGaper" then
		if npc.SubType == 100 then
			...
			elseif sprite:IsEventTriggered("Spawn") then
				npc:PlaySound(SoundEffect.SOUND_MAGGOT_BURST_OUT, 0.6, 0, false, 1)
				mod.throwShit(npc.Position, Vector(0,1):Rotated(rand:RandomInt(360)), -10, -(2+rand:RandomInt(3)), npc, "rottenGaper")
			else
			...
			elseif sprite:IsEventTriggered("Spawn") then
				npc:PlaySound(SoundEffect.SOUND_MAGGOT_BURST_OUT, 0.6, 0, false, 1)
				mod.throwShit(npc.Position, Vector(0,1):Rotated(rand:RandomInt(360)), -10, -(2+rand:RandomInt(3)), npc, "rottenGaper", npc.SubType-2)
			else
				mod:spritePlay(sprite, "Attack03")
			end
		end
