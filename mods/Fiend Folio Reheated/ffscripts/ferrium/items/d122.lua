...
local function snowglobeMap()
	...
	for i=0, size do
		...
		if gridEnt then
			if gridEnt:GetType() == GridEntityType.GRID_SPIKES or gridEnt:GetType() == GridEntityType.GRID_PRESSURE_PLATE or gridEnt:GetType() == GridEntityType.GRID_TELEPORTER or gridEnt:GetType() == GridEntityType.GRID_TRAPDOOR or gridEnt:GetType() == GridEntityType.GRID_STAIRS then
				door = true
			end
		end
...
mod:AddCallback(ModCallbacks.MC_USE_ITEM, function(_, item, rng, player, useflags)
	if useflags == useflags | UseFlag.USE_CARBATTERY then return end
	local room = game:GetRoom()
	local gridCount = 0
	...
	for _,grid in ipairs(mod.GetGridEntities()) do
		if mod.gridToProjectile[grid:GetType()] or mod.gridToProjectileSpecials[grid:GetType()] then
			if grid.CollisionClass == GridCollisionClass.COLLISION_SOLID or grid.CollisionClass == GridCollisionClass.COLLISION_OBJECT or grid.CollisionClass == GridCollisionClass.COLLISION_WALL then
				...
				local proj = mod:turnGridtoProjectile(player, index, dir*0.05, true, ProjectileFlags.NO_WALL_COLLIDE | ProjectileFlags.HIT_ENEMIES | ProjectileFlags.CANT_HIT_PLAYER, true, {GridEntityType.GRID_ROCKB, GridEntityType.GRID_LOCK})
				if proj then
