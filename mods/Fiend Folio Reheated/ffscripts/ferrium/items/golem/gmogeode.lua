...
mod.positivePills = {0, 2, 5, 7, 10, 12, 14, 16, 18, 20, 23, 24, 26, 28, 33, 34, 35, 36, 38, 41, 48}
    ...
    if player:HasTrinket(FiendFolio.ITEM.ROCK.GMO_GEODE) then
        if (not player:GetData().ffsavedata.pillstotake or player:GetData().ffsavedata.pillstotake == 0) and not player:GetData().GMOGeode then
            ...
            if mod.HasTwoGeodes(player) then
                ...
                local num = mod.positivePills[rng:RandomInt(#mod.positivePills)+1]
                player:GetData().queuedSpecificPills = player:GetData().queuedSpecificPills or {}
                table.insert(player:GetData().queuedSpecificPills, num)
            end
        end
    end
end)