...
function mod:fetalStoneFamiliar(player)
    player:CheckFamiliar(mod.ITEM.FAMILIAR.RELATIVE_ROCK, player:HasTrinket(mod.ITEM.ROCK.FETAL_STONE) and 1 or 0, player:GetTrinketRNG(mod.ITEM.ROCK.FETAL_STONE), Isaac.GetItemConfig():GetTrinket(mod.ITEM.ROCK.FETAL_STONE), 0)
end

--[[function mod:fetalStoneUpdate(player, basedata)
	if player:HasTrinket(FiendFolio.ITEM.ROCK.FETAL_STONE) then
		...
		if (not data.fetalStone or not data.fetalStone:Exists()) and mod.ferriumTrinketFamiliarCheck == true then
			...
			table.insert(data.trinketFamiliarGameInit, {var = 1039, key = "fetalStone", init = rock.InitSeed})
		end
	end
end]]
