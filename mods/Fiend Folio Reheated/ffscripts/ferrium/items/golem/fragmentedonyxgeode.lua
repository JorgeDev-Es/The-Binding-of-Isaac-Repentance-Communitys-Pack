...
function mod:fragmentedOnyxGeodeEntDamage(npc, entry, flags, source)
	if mod:isScare(npc) then
		for i = 1, game:GetNumPlayers() do
			local player = Isaac.GetPlayer(i - 1)
			if player:HasTrinket(FiendFolio.ITEM.ROCK.FRAGMENTED_ONYX_GEODE) and mod.HasTwoGeodes(player) then
				local mult = FiendFolio.GetGolemTrinketPower(player, FiendFolio.ITEM.ROCK.FRAGMENTED_ONYX_GEODE)
				local chance = math.min(1, game:GetRoom():GetDevilRoomChance())
				entry.newDamage = entry.newDamage*(1+chance*mult)
