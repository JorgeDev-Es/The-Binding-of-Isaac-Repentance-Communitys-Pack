...
function mod:sandCastleHurt(player, flag, source)
	if flag ~= flag | DamageFlag.DAMAGE_FAKE and flag ~= flag | DamageFlag.DAMAGE_NO_PENALTIES and flag ~= flag | DamageFlag.DAMAGE_IV_BAG and not (source and source.Type == EntityType.ENTITY_SLOT) then
		...
		if player:HasTrinket(FiendFolio.ITEM.ROCK.SAND_CASTLE) then
			...
			local held = false
			if mod:GetRealTrinketId(t0) == FiendFolio.ITEM.ROCK.SAND_CASTLE or mod:GetRealTrinketId(t1) == FiendFolio.ITEM.ROCK.SAND_CASTLE then
				held = true
			end
			...
			if t0 > 0 then
				if t0 == FiendFolio.ITEM.ROCK.SAND_CASTLE then
					...
					removed = true
					sandy = true
				elseif t0 == FiendFolio.ITEM.ROCK.SAND_CASTLE + TrinketType.TRINKET_GOLDEN_FLAG then
					if not data.goldCastle then
						data.goldCastle = true
						player:AddTrinket(FiendFolio.ITEM.ROCK.SAND_CASTLE + TrinketType.TRINKET_GOLDEN_FLAG)
					else
						data.goldCastle = nil
						player:AddTrinket(FiendFolio.ITEM.ROCK.DAMAGED_SAND_CASTLE + TrinketType.TRINKET_GOLDEN_FLAG)
					end
					sandy = true
					removed = true
				else
			...
			if t1 > 0 then
				...
				elseif t1 == FiendFolio.ITEM.ROCK.SAND_CASTLE then
					player:AddTrinket(FiendFolio.ITEM.ROCK.DAMAGED_SAND_CASTLE)
					sandy = true
				elseif t1 == FiendFolio.ITEM.ROCK.SAND_CASTLE + TrinketType.TRINKET_GOLDEN_FLAG then
					if not data.goldCastle then
						data.goldCastle = true
						player:AddTrinket(FiendFolio.ITEM.ROCK.SAND_CASTLE + TrinketType.TRINKET_GOLDEN_FLAG)
					else
		...
		if player:HasTrinket(FiendFolio.ITEM.ROCK.DAMAGED_SAND_CASTLE) and not sandy then
			...
			local held = false
			if mod:GetRealTrinketId(t0) == FiendFolio.ITEM.ROCK.DAMAGED_SAND_CASTLE or mod:GetRealTrinketId(t1) == FiendFolio.ITEM.ROCK.DAMAGED_SAND_CASTLE then
				held = true
			end
			...
			if t0 > 0 then
				if t0 == FiendFolio.ITEM.ROCK.DAMAGED_SAND_CASTLE then
					...
					removed = true
					sandy = true
				elseif t0 == FiendFolio.ITEM.ROCK.DAMAGED_SAND_CASTLE + TrinketType.TRINKET_GOLDEN_FLAG then
					if not data.goldCastle then
						data.goldCastle = true
						player:AddTrinket(FiendFolio.ITEM.ROCK.DAMAGED_SAND_CASTLE + TrinketType.TRINKET_GOLDEN_FLAG)
					else
						data.goldCastle = nil
						player:AddTrinket(FiendFolio.ITEM.ROCK.BROKEN_SAND_CASTLE + TrinketType.TRINKET_GOLDEN_FLAG)
					end
					removed = true
					sandy = true
				else
			...
			if t1 > 0 then
				...
				elseif t1 == FiendFolio.ITEM.ROCK.DAMAGED_SAND_CASTLE then
					player:AddTrinket(FiendFolio.ITEM.ROCK.BROKEN_SAND_CASTLE)
					sandy = true
				elseif t1 == FiendFolio.ITEM.ROCK.DAMAGED_SAND_CASTLE + TrinketType.TRINKET_GOLDEN_FLAG then
					if not data.goldCastle then
						data.goldCastle = true
						player:AddTrinket(FiendFolio.ITEM.ROCK.DAMAGED_SAND_CASTLE + TrinketType.TRINKET_GOLDEN_FLAG)
					else
		...
		if player:HasTrinket(FiendFolio.ITEM.ROCK.BROKEN_SAND_CASTLE) and not sandy then
			...
			local held = false
			if mod:GetRealTrinketId(t0) == FiendFolio.ITEM.ROCK.BROKEN_SAND_CASTLE or mod:GetRealTrinketId(t1) == FiendFolio.ITEM.ROCK.BROKEN_SAND_CASTLE then
				held = true
			end
			...
			if t0 > 0 then
				if t0 == FiendFolio.ITEM.ROCK.BROKEN_SAND_CASTLE then
					...
					removed = true
					sandy = true
				elseif t0 == FiendFolio.ITEM.ROCK.BROKEN_SAND_CASTLE + TrinketType.TRINKET_GOLDEN_FLAG then
					if not data.goldCastle then
						data.goldCastle = true
						player:AddTrinket(FiendFolio.ITEM.ROCK.BROKEN_SAND_CASTLE + TrinketType.TRINKET_GOLDEN_FLAG)
					else
						data.goldCastle = nil
						player:AddTrinket(FiendFolio.ITEM.ROCK.POCKET_SAND + TrinketType.TRINKET_GOLDEN_FLAG)
					end
					removed = true
					sandy = true
				else
			...
			if t1 > 0 then
				...
				elseif t1 == FiendFolio.ITEM.ROCK.BROKEN_SAND_CASTLE then
					player:AddTrinket(FiendFolio.ITEM.ROCK.POCKET_SAND)
					sandy = true
				elseif t1 == FiendFolio.ITEM.ROCK.BROKEN_SAND_CASTLE + TrinketType.TRINKET_GOLDEN_FLAG then
					if not data.goldCastle then
						data.goldCastle = true
						player:AddTrinket(FiendFolio.ITEM.ROCK.BROKEN_SAND_CASTLE + TrinketType.TRINKET_GOLDEN_FLAG)
					else
		...
		if player:HasTrinket(FiendFolio.ITEM.ROCK.BROKEN_SAND_CASTLE) or player:HasTrinket(FiendFolio.ITEM.ROCK.DAMAGED_SAND_CASTLE) then
			...
			if t0 > 0 then
				if t0 == FiendFolio.ITEM.ROCK.BROKEN_SAND_CASTLE or t0 == FiendFolio.ITEM.ROCK.DAMAGED_SAND_CASTLE then
					player:AddTrinket(FiendFolio.ITEM.ROCK.SAND_CASTLE)
				elseif t0 == FiendFolio.ITEM.ROCK.BROKEN_SAND_CASTLE + TrinketType.TRINKET_GOLDEN_FLAG or t0 == FiendFolio.ITEM.ROCK.DAMAGED_SAND_CASTLE + TrinketType.TRINKET_GOLDEN_FLAG then
					player:AddTrinket(FiendFolio.ITEM.ROCK.SAND_CASTLE + TrinketType.TRINKET_GOLDEN_FLAG)
				else
					player:AddTrinket(t0)
				end
			...
			if t1 > 0 then
				if t1 == FiendFolio.ITEM.ROCK.BROKEN_SAND_CASTLE or t1 == FiendFolio.ITEM.ROCK.DAMAGED_SAND_CASTLE then
					player:AddTrinket(FiendFolio.ITEM.ROCK.SAND_CASTLE)
				elseif t1 == FiendFolio.ITEM.ROCK.BROKEN_SAND_CASTLE + TrinketType.TRINKET_GOLDEN_FLAG or t1 == FiendFolio.ITEM.ROCK.DAMAGED_SAND_CASTLE + TrinketType.TRINKET_GOLDEN_FLAG then
					player:AddTrinket(FiendFolio.ITEM.ROCK.SAND_CASTLE + TrinketType.TRINKET_GOLDEN_FLAG)
				else
					player:AddTrinket(t1)
				end
