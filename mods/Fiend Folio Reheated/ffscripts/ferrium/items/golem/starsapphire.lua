...
function mod:starSapphireFamiliar(player)
    player:CheckFamiliar(mod.ITEM.FAMILIAR.STAR_SAPPHIRE_GEM, player:HasTrinket(mod.ITEM.ROCK.STAR_SAPPHIRE) and 1 or 0, player:GetTrinketRNG(mod.ITEM.ROCK.STAR_SAPPHIRE), Isaac.GetItemConfig():GetTrinket(mod.ITEM.ROCK.STAR_SAPPHIRE), 0)
end

--[[function mod:starSapphireUpdate(player, basedata)
	if player:HasTrinket(FiendFolio.ITEM.ROCK.STAR_SAPPHIRE) then
		local data = basedata.ffsavedata
		if (not data.starSapphire or not data.starSapphire:Exists()) and mod.ferriumTrinketFamiliarCheck == true then
			...
			data.trinketFamiliarGameInit = data.trinketFamiliarGameInit or {}
			table.insert(data.trinketFamiliarGameInit, {var = FamiliarVariant.STAR_SAPPHIRE_GEM, key = "starSapphire", init = rock.InitSeed})
		end
	end
end]]
