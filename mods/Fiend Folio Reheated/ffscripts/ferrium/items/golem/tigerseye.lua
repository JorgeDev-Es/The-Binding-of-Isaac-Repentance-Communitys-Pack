...
function mod:tigersEyeFamiliar(player)
    player:CheckFamiliar(mod.ITEM.FAMILIAR.TIGERS_EYE, player:HasTrinket(mod.ITEM.ROCK.TIGERS_EYE) and 1 or 0, player:GetTrinketRNG(mod.ITEM.ROCK.TIGERS_EYE), Isaac:GetItemConfig():GetTrinket(mod.ITEM.ROCK.TIGERS_EYE), 0)
end

--[[function mod:tigersEyeUpdate(player, basedata)
	if player:HasTrinket(FiendFolio.ITEM.ROCK.TIGERS_EYE) then
        local data = basedata.ffsavedata
		if (not data.tigersEye or not data.tigersEye:Exists()) and mod.ferriumTrinketFamiliarCheck == true then
			...
			table.insert(data.trinketFamiliarGameInit, {var = FamiliarVariant.TIGERS_EYE, key = "tigersEye", init = rock.InitSeed})
		end
	end
end]]
