...
function mod:focusCrystalEntDamage(ent, entry, flags, source)
	if ent.Type ~= 33 then
		for i = 1, game:GetNumPlayers() do
			local player = Isaac.GetPlayer(i - 1)
			if player:HasTrinket(FiendFolio.ITEM.ROCK.FOCUS_CRYSTAL) then
				...
				if ent.Position:Distance(player.Position) < 100 then
					sfx:Play(mod.Sounds.FocusCrystal, 0.4, 0, false, 1)
					local rng = player:GetTrinketRNG(FiendFolio.ITEM.ROCK.FOCUS_CRYSTAL)
					local poof = Isaac.Spawn(mod.FF.FocusCrystalPoof.ID, mod.FF.FocusCrystalPoof.Var, mod.FF.FocusCrystalPoof.Sub, ent.Position+mod:shuntedPosition(5, rng), Vector.Zero, ent):ToEffect()
					if flags ~= flags | DamageFlag.DAMAGE_FIRE then --Some damage callback somewhere that's messing with this
						entry.newDamage = entry.newDamage*(1+0.2*mult)
						...
						poof:GetSprite().Rotation = rng:RandomInt(360)
					end
				end
			end
		end
