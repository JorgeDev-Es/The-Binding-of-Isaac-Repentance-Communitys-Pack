...
mod:AddCallback(ModCallbacks.MC_NPC_UPDATE, function(_, npc)
	...
	if data.FFFuzzyPoop then
		data.FuzzySkuzzes = data.FuzzySkuzzes or {}
		for i, entity in pairs(data.FuzzySkuzzes) do
			if not entity:Exists() then
				data.FuzzySkuzzes[i] = nil
			end
		end
		if npc.FrameCount % 60 == 0 and #data.FuzzySkuzzes < 3 and not data.PoopIsBroken then
			local skuzz = Isaac.Spawn(3, FamiliarVariant.ATTACK_SKUZZ, 0, npc.Position, RandomVector():Resized(3), npc):ToFamiliar()
			skuzz:ClearEntityFlags(EntityFlag.FLAG_APPEAR)
			table.insert(data.FuzzySkuzzes, skuzz)
		end
		data.PoopIsBroken = npc.HitPoints == 1
	end
	...
	for i, entity in pairs(Isaac.FindInRadius(effect.Position, 1, EntityPartition.ENEMY)) do
		if entity.Type == EntityType.ENTITY_POOP then
			...
			elseif data.FFEvilPoop then
				...
				effect:GetSprite():LoadGraphics()
			elseif data.FFFuzzyPoop then
				effect:GetSprite():ReplaceSpritesheet(0, "gfx/grid/grid_fuzzypoopgibs.png")
				effect:GetSprite():LoadGraphics()
			end
		end
	end
...
mod:AddCallback(ModCallbacks.MC_POST_ENTITY_REMOVE, function(_, entity)
	if entity.Variant == FamiliarVariant.DIP and entity.SubType == 673 then --This triggers twice and I have no idea why or how to stop it
		local skuzz = Isaac.Spawn(3, FamiliarVariant.ATTACK_SKUZZ, 0, entity.Position, Vector.Zero, entity):ToFamiliar()
		skuzz:ClearEntityFlags(EntityFlag.FLAG_APPEAR)
	end
end, EntityType.ENTITY_FAMILIAR)

local function IsValidDipSpellTarget(player)
	...
	for i, entity in pairs(Isaac.FindInRadius(player.Position, 80, EntityPartition.PICKUP)) do
		if entity.Type == EntityType.ENTITY_PICKUP and entity.Variant == PickupVariant.PICKUP_COLLECTIBLE and entity.SubType == FiendFolio.ITEM.COLLECTIBLE.DADS_DIP then
			return true
		end
	end
end
...
mod:AddCallback(ModCallbacks.MC_POST_PEFFECT_UPDATE, function(_, player)
	...
	if savedata.Poops and (savedata.Poops[1] == "DIP" or savedata.StoredPoop == "DIP") and IsValidDipSpellTarget(player) then
	...
	elseif data.DipSpellAura and data.DipSpellAura:Exists() then
		if not data.DipSpellAura:GetSprite():IsPlaying("Disappear") then
			data.DipSpellAura:GetSprite():Play("Disappear")
		elseif data.DipSpellAura:GetSprite():GetFrame() == 7 then
...
if FiendFolio.WackyPoopsEnabled then
	...
	mod:AddSpellToPool("FUZZY", "Common", 0.5)
	mod:AddSpellToPool("FUZZY", "Uncommon", 0.5)
end
...
mod:AddPoopSpellCallback(function(_, spell, player)
	...
	for i, entity in pairs(Isaac.FindInRadius(player.Position, 80, EntityPartition.PICKUP)) do
		if entity.Type == EntityType.ENTITY_PICKUP and entity.Variant == PickupVariant.PICKUP_COLLECTIBLE and entity.SubType == FiendFolio.ITEM.COLLECTIBLE.DADS_DIP then
			for i = 1, mod:RandomInt(20, 30) do
				FiendFolio.scheduleForUpdate(function()
					player:ThrowFriendlyDip(672, entity.Position, entity.Position + Vector(mod:RandomInt(-50,50),mod:RandomInt(-50,50)))
					SFXManager():Play(SoundEffect.SOUND_PLOP)
					local effect = Isaac.Spawn(EntityType.ENTITY_EFFECT, EffectVariant.POOF01, 0, entity.Position, Vector.Zero, player):ToEffect()
					effect.Scale = 0.1
				end, i*2 + mod:RandomInt(0,3))
			end
			entity:Remove()
		end
	end
...
mod.SpellSprites["FUZZY"] = {}
for j = 1, 4 do
	mod.SpellSprites["FUZZY"][mod.SpellAnimations[j]] = Sprite()
	mod.SpellSprites["FUZZY"][mod.SpellAnimations[j]]:Load("gfx/ui/ui_ffpoop.anm2", true)
	mod.SpellSprites["FUZZY"][mod.SpellAnimations[j]]:Play(mod.SpellAnimations[j])
	mod.SpellSprites["FUZZY"][mod.SpellAnimations[j]]:SetFrame(4)
end
...
mod:AddPoopSpellCallback(function(_, spell, player)
	...
	poop:GetData().FFFuzzyPoop = true
	poop:GetSprite():ReplaceSpritesheet(0, "gfx/grid/grid_fuzzy_poop.png")
	poop:GetSprite():LoadGraphics()
	player:TryHoldEntity(poop)
	SFXManager():Play(SoundEffect.SOUND_POOPITEM_HOLD)
end, "FUZZY")