...
mod:AddCallback(ModCallbacks.MC_USE_ITEM, function(_, id, rng, player, useflags, activeslot, customvardata)
    ...
    for _, t in ipairs(trinkets) do
        ...
        if t:GetData().AzuriteSpin == 1 then            --Car battery check
            ...
            local fadeT = 8
            if mod:IsGoldTrinket(t.SubType) then
                for i=1, fadeT do
            ...
            mod.scheduleForUpdate(function()
                if t:Exists() and t:GetData().AzuriteSpin then
                    local id = mod:GetRealTrinketId(t.SubType)
                    ...
                    if id <= 0 then                                                         --vanish
                        ...
                        local price = t.Price
                        t:Morph(5, 350, id + mod:GetTrinketFlags(t.SubType), false)
