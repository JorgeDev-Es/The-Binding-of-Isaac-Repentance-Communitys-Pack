...
mod:AddCallback(ModCallbacks.MC_POST_ENTITY_REMOVE, function(_, proj)
  ...
  if data.punisher then
    ...
    if (proj.SpawnerEntity and mod:isFriend(proj.SpawnerEntity)) or proj:ToProjectile():HasProjectileFlags(ProjectileFlags.CANT_HIT_PLAYER) then
      flame:GetData().fireJetNoHurtPlayer = true
    end
...
mod:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, function(_, npc, dmg, dmgType, dmgSrc, dmgCountDown)
  if dmgSrc.Entity and dmgSrc.Entity:GetData().fireJetNoHurtPlayer then return false end
