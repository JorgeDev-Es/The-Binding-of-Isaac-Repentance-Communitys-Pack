...
FiendFolio.StatusEffectDesc = {
	BERSERK = "#{{ffBerserk}} Berserked enemies periodically switch targets (including enemies), take bonus damage, move faster and cannot die during the duration",
	BRUISE = "#{{ffBruise}} Bruised enemies take bonus damage from all sources of damage based on the number of stacks applied",
	DOOM = "#{{ffDoom}} Doomed enemies gain a counter that goes down every time they take damage#{{ffDoom}} Upon reaching zero the enemy takes a large amount of damage, removing the doomed status",
	DROWSY_AND_SLEEP = "#{{ffSleeping}} Drowsy enemies will slow down and eventually fall asleep#{{ffSleeping}} Damaging a sleeping enemy deals double damage but wakes them up",
	HEMORRHAGE = "#{{ffHemorrhage}} Hemorrhaging enemies periodically take damage, spew blood tears randomly around themselves and leave blood creep on the ground that damages enemies",
	MARTYR = "#{{ffMartyr}} Martyrized enemies will leave behind a short-lived ghost with an aura#{{ffMartyr}} Standing in the aura grants tears up, damage up and homing",
	SEW = "#{{ffSew}} Sewn enemies reflect damage taken onto all other enemies inflicted with sewn",
	MULTIEUCLIDEAN = "#{{ffmultieuclidean}} Enemies that are multi-euclidean can be shot through, doubling tears",
}
...
FiendFolio.ExternalDescriptions = {
	COLLECTIBLE = {
		...
		},
		{
			...
			EID = {
				Desc = "Luck-affected chance to shoot sewing needle tears that pierce and inflict enemies with sewn",
				StatusEffects = {FiendFolio.StatusEffectDesc.SEW},
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
					Encyclopedia.ItemPools.POOL_TREASURE,
		...
		},
		{
			...
			EID = {
				...
				StatusEffects = {
					FiendFolio.StatusEffectDesc.BRUISE,
					FiendFolio.StatusEffectDesc.HEMORRHAGE,
				},
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.MALICE_REFORM,
			Encyclopedia = Encyclopedia and {
				Pools = {},
				Hide = true
			},
		...
		},
		{
			...
			Encyclopedia = Encyclopedia and {
				...
				Wiki = "\1 +1 black heart#Guarantees a curse on every floor#Replaces curses with special new kinds of potentially beneficial curses#Curse of the Imp: Heart pickups have a chance to be replaced with Fiend's minions#Curse of the Stone: Your speed is increased or lowered depending on how many trinkets and pocket items you are carrying#Curse of the Sun: All stats up. If you get hit, all stats down. After clearing 3 rooms without taking damage, the buff will be restored#Curse of the Swine: Enemies have a chance to drop pennies on death, but all pennies fade away after some time#Curse of the Ghost: Raindrops that hurt both you and enemies fall from the sky. Drops will often spawn close to enemies and home in on them. Rain intensity increases as your health decreases#Curse of the Scythe: Hurting enemies builds rage. Once rage reaches a certain level, you enter rage mode which greatly increases your fire rate but also applies the Bruised status effect, reducing your invincibility frames.#Curse of the Master: Hurting enemies without getting hurt yourself causes your style level to increase. A high style level will make all pickups and items you find more valuable. A low style level will do the opposite",
			},
		},
		{
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.CLEAR_CASE_PASSIVE,
			Encyclopedia = Encyclopedia and {
				Pools = {},
				Hide = true
			},
		},
		{
			...
			EID = {
				Desc = "Enemies killed by tears will turn into martyrs",
				Transformations = "10",
				StatusEffects = {FiendFolio.StatusEffectDesc.MARTYR},
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
					Encyclopedia.ItemPools.POOL_ANGEL,
		...
		},
		{
			...
			EID = {
				Desc = "Inflicts all enemies with drowsy",
				Transformations = "12",
				StatusEffects = {FiendFolio.StatusEffectDesc.DROWSY_AND_SLEEP},
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
					Encyclopedia.ItemPools.POOL_TREASURE,
		...
		},
		{
			...
			EID = {
				...
				StatusEffects = {
					Hide = true,
					FiendFolio.StatusEffectDesc.BERSERK,
					FiendFolio.StatusEffectDesc.BRUISE,
					FiendFolio.StatusEffectDesc.DOOM,
					FiendFolio.StatusEffectDesc.DROWSY_AND_SLEEP,
					FiendFolio.StatusEffectDesc.HEMORRHAGE,
					FiendFolio.StatusEffectDesc.MARTYR,
					FiendFolio.StatusEffectDesc.SEW,
					FiendFolio.StatusEffectDesc.MULTIEUCLIDEAN,
				},
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
		...
		},
		{
			...
			EID = {
				Desc = "Grants an extra life#On death, the player respawns as Fiend in the previous room"
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
					Encyclopedia.ItemPools.POOL_DEVIL,
		...
		},
		{
			...
			EID = {
				Desc = "Luck-affected chance to fire bullet tears that inflict enemies with bruising",
				StatusEffects = {FiendFolio.StatusEffectDesc.BRUISE},
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
					Encyclopedia.ItemPools.POOL_TREASURE,
		...
		},
		{
			...
			EID = {
				Desc = "Menace familiar that charges at enemies#Double-tap the shoot button to possess the nearest enemy and inflict them with berserk#Double-tap the shoot button to exit the enemy and fire a spectral tear barrage",
				StatusEffects = {FiendFolio.StatusEffectDesc.BERSERK},
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
					Encyclopedia.ItemPools.POOL_TREASURE,
		...
		},
		{
			...
			EID = {
				Desc = "\1 +1 rotten heart#\1 +0.2 speed up#\1 +0.3 damage up#\1 +2.5 range up",
				Transformations = "2",
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
		...
		},
		{
			...
			EID = {
				Desc = "Luck-affected chance to fire tears that inflict enemies with hemorrhage",
				StatusEffects = {FiendFolio.StatusEffectDesc.HEMORRHAGE},
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
					Encyclopedia.ItemPools.POOL_TREASURE,
		...
		},
		{
			...
			EID = {
				Desc = "Luck-affected chance to fire tears that inflict enemies with doom",
				StatusEffects = {FiendFolio.StatusEffectDesc.DOOM},
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
					Encyclopedia.ItemPools.POOL_TREASURE,
		...
		},
		{
			...
			EID = {
				Desc = "Luck-affected chance to fire tears that inflict enemies with drowsy",
				StatusEffects = {FiendFolio.StatusEffectDesc.DROWSY_AND_SLEEP},
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.GOLDEN_PLUM_FLUTE,
			EID = {
				Desc = "Summons friendly Golden Plum in the current room for 10 seconds",
				Transformations = "3",
			},
			Encyclopedia = Encyclopedia and {
				Pools = {}
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.DOGBOARD,
			EID = {				-- NO DESC
				Desc = "--"
			},
			Encyclopedia = Encyclopedia and {
		...
		},
		{
			...
			EID = {
				Desc = "Increases the chance for immoral hearts, cursed pennies, dire chests, spicy keys, copper bombs and cursed batteries to spawn"
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
					Encyclopedia.ItemPools.POOL_SHOP,
		...
		},
		{
			...
			EID = {
				Desc = "Grants 3 homing gems that can be cast while attacking and stick to enemies, inflicting damage and bruising#Has special synergies with certain items",
				StatusEffects = {FiendFolio.StatusEffectDesc.BRUISE},
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
		...
		},
		{
			...
			EID = {
				Desc = "Familiar that drops 1 set of spicy keys every 7 rooms"
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
					Encyclopedia.ItemPools.POOL_CURSE
		...
		},
		{
			...
			EID = {
				Desc = "\1 +2.5 Range up#Spawns a bunch of skuzzes",
				Transformations = "6",
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
		...
		},
		{
			...
			EID = {
				Desc = "Has a chance to fire multi-euclidean tears",
				StatusEffects = {FiendFolio.StatusEffectDesc.MULTIEUCLIDEAN},
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
					Encyclopedia.ItemPools.POOL_TREASURE,
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.MY_STORY_NULL_TEARS,
			Encyclopedia = Encyclopedia and {
				Pools = {},
				Hide = true
			},
		},
		{
			...
			EID = {
				Desc = "\1 +0.5 Damage up#\1 +1.5 Range up"
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
					Encyclopedia.ItemPools.POOL_BOSS,
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.SMASH_TROPHY,
			EID = {
				Desc = "\1 +1 damage up#Enemies receive more knockback the less health they have"
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
					Encyclopedia.ItemPools.POOL_TREASURE,
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.BOTTLE_OF_WATER,
			EID = {
				Desc = "\1 +2 soul hearts#Gulps held trinkets#Spawns a pill"
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
					Encyclopedia.ItemPools.POOL_BOSS,
		...
		{
			ID = Isaac.GetItemIdByName(" Hold"),
			EID = {
				Desc = "Preserves one poop bomb inside and can be used later."
			},
			Encyclopedia = Encyclopedia and {
				Pools = {},
				Hide = true
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.GAMMA_GLOVES,
			EID = {				
				Desc = "On use, creates a small shockwave around the player, pushing nearby enemies away"
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
					Encyclopedia.ItemPools.POOL_GREED_TREASURE,
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.SHREDDER,
			EID = {
				Desc = "On use, turns item pedestals into pickups (like Tainted Cain)"
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
					Encyclopedia.ItemPools.POOL_SHOP,
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.FISTFUL_OF_ASH,
			EID = {
				Desc = "\1 +3 Bombs#Tinted rocks have odder visuals#Breaking tinted rocks spawns half a heart and grants a fading damage up#More tinted rocks#Breaking tinted rocks also has a 50% chance to remove a broken heart#\2 Gain a broken heart each floor"
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
					Encyclopedia.ItemPools.POOL_SECRET,
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.FIENDS_THIRD_LEG,
			EID = {
				Desc = "\1 3.33x Damage#\1 1.5x Tears#Homing Shots#Tears leave a trail of damaging creep#Tears leave a Minion on impact"
			},
			Encyclopedia = Encyclopedia and {
				Pools = {},
				Hide = true
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.LOADED_D6,
			EID = {
				Desc = "On use, rerolls pedestal items in the current room into copies of items you're already holding#Will prioritize items that aren't useless when stacked"
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
					Encyclopedia.ItemPools.POOL_TREASURE,
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.ISAAC_DOT_CHR,
			EID = {
				Desc = "Grants an extra life#On death, the player respawns with some of their items replaced with TMTRAINER items"
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
					Encyclopedia.ItemPools.POOL_SECRET,
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.WORM_INSIGNIA,
			EID = {
				Desc = "\1 +0.02 speed up"
			},
			Encyclopedia = Encyclopedia and {
				Pools = {}
			},
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.DADS_BATTERY,
			EID = {
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.TORTURE_COOKIE,
			EID = {
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.ISAACD_EULOGY,
			EID = {
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.BRICK_FIGURE,
			EID = {
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.MOONBEAM,
			EID = {
				Desc = ""
			},
			Encyclopedia = Encyclopedia and {
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.GOLDSHI_LUNCH,
			EID = {
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.TWINKLE_OF_CONTAGION,
			EID = {
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.DUSTY_D10,
			EID = {
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.TOKEN_BAG,
			EID = {
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.GREEN_ORANGE,
			EID = {
				Desc = "\1 +0.7 damage up#\1 +0.5 tears up#Spawns a soul heart on pickup"
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
					Encyclopedia.ItemPools.POOL_BOSS,
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.REHEATED_PIZZA,
			EID = {
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.HEDONISTS_COOKBOOK,
			EID = {
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.ERRORS_CRAZY_SLOTS,
			EID = {
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.SPELLING_BEE,
			EID = {
				Desc = "Grants stat boosts and debuffs based on the characters in the seed#If a letter is from a stat based word such as DAMAGE, it grants those stats#The same is true for pickups#Numbers will grant a minor all stats down"
			},
			Encyclopedia = Encyclopedia and {
				Pools = {}
			},
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.SCULPTED_PEPPER,
			EID = {
		...
		{
			ID = FiendFolio.ITEM.COLLECTIBLE.TOO_MANY_OPTIONS,
			EID = {
				Desc = "Special room doors quickly cycle between 3 random special room types until unlocked or entered"
			},
			Encyclopedia = Encyclopedia and {
				Pools = {
					Encyclopedia.ItemPools.POOL_SHOP,
...
-- COLLECTIBLES
for _, itemEntry in ipairs(FiendFolio.ExternalDescriptions.COLLECTIBLE) do
	-- EID
	local id = itemEntry.ID
	if itemEntry.EID then
		local desc = itemEntry.EID.Desc
		if itemEntry.EID.StatusEffects and not itemEntry.EID.StatusEffects.Hide then
			for _, eff in ipairs(itemEntry.EID.StatusEffects) do
				desc = desc .. eff
			end
		end
	...
	if Encyclopedia and itemEntry.Encyclopedia then
		Encyclopedia.AddItem({
			...
			UnlockFunc = function(self)
				if FiendFolio.IsCollectibleLocked(itemEntry.ID) then
					self.Desc = "Locked!"
					return self
				end
			end,
...
if EID then
	...
	EID:addIcon("ffmultieuclidean", "StatusEffectMultiEuclidean", -1, 10, 9, 0, 1, icons)
	EID:addIcon("ffMartyr", "StatusEffects", 8, 10, 9, 0, 1, icons)
