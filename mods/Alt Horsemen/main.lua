...
local loadText = "Alt Horsemen v5.72 (COMPLETE)"
...
local function FloorVerify()
	...
	if not backwards then
		...
		if FFGRACE then
			...
			if FFGRACE.STAGE.Grotto:IsStage() and not bossSeen.w2 then
				return w2.name .. " Grotto"
			end
		end
...
if StageAPI and firstLoaded then	
	mod.StageAPIBosses = {
		...
		StageAPI.AddBossData(w2.name .. " Grotto", {
			Name = w2.name .. " Grotto",
			Portrait = "gfx/bosses/war2/grotto/portrait_war2_grotto.png",
			Offset = Vector(0,-15),
			Bossname = w2.bossName,
			Weight = w2.weight,
			Rooms = StageAPI.RoomsList("AHWarRooms", require("resources.luarooms.boss_war2")),
        ...
        if room:GetType() == RoomType.ROOM_BOSS then
			...
			for i, entity in ipairs(Isaac.GetRoomEntities()) do
				...
				elseif entity.Type == w2.id and entity.Variant == w2.variant then
					...
					StageAPI.SetBossEncountered(w2.name .. " Grotto")
					break
				--DEATH
