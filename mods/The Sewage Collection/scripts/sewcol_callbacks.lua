...
--[[##########################################################################
########################### EID INDICATOR OVERRIDE ###########################
##########################################################################]]--
if EID and not TCC_API.EIDRenderOverride then
    local oldColor = Color(1,1,1,0.5)
    oldColor:SetColorize(1, 1.4, 1.8, 1)

    local renderColor = Color(1,1,1,1)
    renderColor:SetColorize(1, 1.4, 1.8, 1)

    TCC_API.IS_MIRRORED = false
    function TCC_API:onNewRoom()
        TCC_API.IS_MIRRORED = Game():GetLevel():GetCurrentRoom():IsMirrorWorld()
    end

    TCC_API:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, TCC_API.onNewRoom)

    function TCC_API:EIDRenderOverride()
        if EID.lastDescriptionEntity and EID.Config["Indicator"] ~= "none" and EID.Config["Indicator"] ~= "arrow" then
            if EID.lastDescriptionEntity.GetData and EID.lastDescriptionEntity:GetData().SEWCOL_MIRRORED then
                local sprite = EID.lastDescriptionEntity:GetSprite()
                local entityPos = Isaac.WorldToScreen(EID.lastDescriptionEntity.Position)

                if TCC_API.IS_MIRRORED then
                ...
                if EID.lastDescriptionEntity.Type == 5 and EID.lastDescriptionEntity.Variant == 100 then
                    sprite:RenderLayer(1, entityPos, Vector(0,0), Vector(0,0))
                else
                    sprite:Render(entityPos, Vector(0,0), Vector(0,0))
                end
    ...
    TCC_API:AddCallback(ModCallbacks.MC_POST_RENDER, TCC_API.EIDRenderOverride)
end