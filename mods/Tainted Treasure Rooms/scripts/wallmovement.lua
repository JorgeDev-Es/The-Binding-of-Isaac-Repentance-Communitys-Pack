--Originally written for Fiend Folio
...
TaintedTreasure.GuwahDirections = {"Left", "Right", "Up", "Down"} --Strings isn't probably the "right" way of doing things but it makes things alot more readable and easier to grasp
TaintedTreasure.WallStickerOffset = Vector(-20,-20) --Standardized offset for traveling along the "grid-edges", sets you to the top left corner of a grid
local wallStickOffset = TaintedTreasure.WallStickerOffset
...
function mod:GetOrientationFromVector(vec) --Ideal for translating standard vector into WallMovement initialization params (Lil Slugger's sawblades use this)
    local dir
    local cc
...
function mod:WallHuggerInit(npc, startDirection, isCounterclockwise, gridthreshold, snapspeed)
    ...
    data.Timestep = math.max(1, math.floor((data.TargetPos - data.StartPos):Length()/snapspeed))
