...
return function(DSSModName, DSSCoreVersion, MenuProvider)
    ...
    function dssmod:post_render()
		...
		if displayedDSSMenuMsg == false and TaintedTreasure:IsSettingOn(TaintedTreasure:GetSaveData().config.ShowControls) then
	...
	dssmod.startPopupButton = {
		...
		load = function()
			return TaintedTreasure:GetSaveData().config.ShowControls or 1
		end,

		store = function(var)
			TaintedTreasure:GetSaveData().config.ShowControls = var
		end,

		tooltip = {strset = {'shows dss', 'controls when', 'entering a', 'run for the', 'first time'}},
		displayif = sharedButtonDisplayCondition
	}
